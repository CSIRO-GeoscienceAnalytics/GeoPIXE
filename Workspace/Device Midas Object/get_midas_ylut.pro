function get_midas_ylut, file, output=output, strip=strip, error=error

; Read a midas Y lookup table file that lists the first Y value
...
endelse
ylut = 0L
print,'get_midas_ylut: try YLUT = ',f0
if file_test(f0) then begin
	...
	error = 0
	return, ylut
endif else print,'get_midas_ylut: YLUT not found = ',f0
return, 0L

bad:
	close_file, lun
	print,'get_midas_ylut: error reading YLUT = ',f0
	return, 0L
end

pro midas_accumulate_dtfx_3D, t,x,y,z,ste,veto,fx,n,n_fx, flux_mode, flux_scale, xrange,yrange,zrange, $
			dta,dtb, dead, flux=flux, dwell=dwell, xcompress=xcompress, ycompress=ycompress, $
			spectrum=spectrum, image=image, pseudo=pseudo, error=err
;
;  This assumes x,y,z,t,veto,pseudo	uintarr(n) (pseudo only used for /image)
;				fx			fltarr(n_fx,n)
;				flux_mode	0=Epics PV, 1=FC0, 2=FC1
;				flux_scale	units for flux scaling
;				dta,dtb		dead-time calibration (ms)
...
;	xrange,yrange,zrange	dimensions for image mode flux array
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'midas_accumulate_dtfx_3D',['IDL run-time error caught.', '', $
...
if size(dta,/tname) ne 'FLOAT' then begin sb='dta' & goto, bad & endif
if size(dtb,/tname) ne 'FLOAT' then begin sb='dtb' & goto, bad & endif
...
err = call_external( geopixe_library(), geolib_name( 'midas_accumulate_dtfx_3d'), cdecl=geolib_cdecl(), $
		image, t,x,y,z,ste,veto,pseudo,fx,long(n),long(n_fx), long(flux_mode), flux_scale, $
		long(xcompress),long(ycompress), long(xrange),long(yrange),long(zrange), $
		dta,dtb, flux, dead, dwell, value=value )
if err then gprint,' midas_accumulate_dtfx_3D: Error in Fortran - bad number of arguments'
return

bad:
	gprint,' midas_accumulate_dtfx_3D: Error - bad type for argument: '+sb
	err = 1L
	return
end

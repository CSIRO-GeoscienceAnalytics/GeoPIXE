pro midas_accumulate_dtfx, x,y,ste,veto,fx,n,n_fx, flux_scale, xrange,yrange,n_flux, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'midas_accumulate_dtfx',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'midas_accumulate_dtfx'), cdecl=geolib_cdecl(), $
		...
		flux, dwell, value=value )
if err then gprint,' midas_accumulate_dtfx: Error in Fortran - bad number of arguments'
return

bad:
	gprint,' midas_accumulate_dtfx: Error - bad type for argument: '+sb
	err = 1L
	return
end

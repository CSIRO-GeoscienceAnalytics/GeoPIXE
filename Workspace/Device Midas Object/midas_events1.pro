function midas_events1, ev,n_buffer, channel_on,nc, e,t,x,y,ste,veto,tags,n_events,n, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'midas_events1',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'midas_events1'), cdecl=geolib_cdecl(), $
...
if err then begin
	print, 'midas_events1 Fortran error'
	print,'		tag, length, ibranch, skip =',tag, length, ibranch, skip
endif
return, err

bad:
	gprint,'midas_events1: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

...
pro flatten_to_flux0_image_plugin, p, ii, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = '* Flatten to Flux0 Image Plugin'		; return the menu title for this plugin
...
q = where( el_name eq 'Flux0', nq)			; flatten based on "Flux0" image
...
r = options_popup( title='Normalize to Flux0 Options', text=text, initial_text=initial_text, help_text=help_text, $
...
print,'Flatten to flux0; base = ',base
scale =  flux_flatten( (*pimg)[*,*,q[0]], base=base, qbad=qb)
nqb = n_elements(qb)
...
if (mode eq 1) and (qb[0] ne -1) then begin					; zero dodgy flux pixels
	...
	s = str_tidy(nqb)
	history = 'Flatten to "Flux0" plugin, base='+str_tidy(base)+' (zero '+s+' bad pixels)'
	...
	s = str_tidy(nqb)
	s2 = str_tidy(nqz)
	history = 'Flatten to "Flux0" plugin, base='+str_tidy(base)+' (interpolate '+s+' bad pixels, '+s2+' remain)'

endif else begin
	s = str_tidy(nqb)
	history = 'Flatten to "Flux0" plugin, base='+str_tidy(base)+' ('+s+' bad pixels not changed)'
endelse
...
bad_element:
	warning,'flatten_to_flux0_image_plugin','Could not find "Flux0".'
	return
end

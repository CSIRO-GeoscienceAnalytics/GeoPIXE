...
;
; name()				return name of device (e.g. "pnc_cat_hdf_DEVICE").
...
pro pnc_cat_hdf_DEVICE::cleanup
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'pnc_cat_hdf_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
pro pnc_cat_hdf_device::set_options, p, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_device::set_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
function pnc_cat_hdf_device::get_options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_device::get_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
pro pnc_cat_hdf_device::read_options, unit, options=options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_device::read_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
pro pnc_cat_hdf_device::write_options, unit, options=p, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_device::write_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
function pnc_cat_hdf_device::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	if H5F_is_hdf5(file) eq 0 then begin
		warning,'pnc_cat_hdf_DEVICE::get_header_info','Not a valid HDF5 file: '+file
		self.header.error = 1
		return, self.header
	endif
...
pro pnc_cat_hdf_DEVICE::update_header_info, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::update_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
pro pnc_cat_hdf_DEVICE::check_pv_list, plist
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::check_pv_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
pro pnc_cat_hdf_DEVICE::flux_scan, unit, evt_file, PV_list, IC_name, IC_val, IC_vunit, dwell=dwell, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::flux_scan',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
bad_io:
	warning,'pnc_cat_hdf_DEVICE::flux_scan','HDF file I/O error.'
	error = 1
	return
end
...
function pnc_cat_hdf_DEVICE::read_setup, unit, xrange, yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_nsls_5, nsls_IC_value_index, nsls_flux_scale
	...
	if nq eq 0 then begin
		warning,'pnc_cat_hdf_DEVICE::read_setup','No "Detectors" dataset found.'
		H5F_close, hdf5_id
		goto, bad_io
	endif
...
;	endif																; This assumes PV values are RATES not counts.
	...
	if n_det eq 0 then begin
		warning,'pnc_cat_hdf_DEVICE::read_setup','No "MCA" datasets found in group.'
		H5F_close, hdf5_id
		goto, bad_io
	endif
	i_spectra = 0
...
function pnc_cat_hdf_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
function pnc_cat_hdf_DEVICE::get_dwell, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::get_dwell',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
function pnc_cat_hdf_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
case name of
	'pnc_cat_hdf_spec': begin						; 41
		p2 = 0L
		if H5F_is_hdf5( file) eq 0 then begin 
			warning,'pnc_cat_hdf_DEVICE::get_import_list','Not a valid HDF5 file: '+file
		...
		if nq eq 0 then begin
			warning,'pnc_cat_hdf_DEVICE::get_import_list','No "MCA" datasets found in group.'
			H5F_close, file_id
			goto, pnc_cat_hdf_evt_done
		endif
...
pnc_cat_hdf_evt_done:
	...
	'pnc_cat_hdf_evt': begin							; 9
		warning,'pnc_cat_hdf_DEVICE::import_spec',['"pnc_cat_hdf_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
		end
	endcase
	return, p2
end
...
function pnc_cat_hdf_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_41 = define(/import)			; PNC-CAT new HDF5 image file
		opt_41.name =		'pnc_cat_hdf_spec'		; unique name of import
		opt_41.title =		'Extract E from PNC_CAT image HDF5 file'
	...
	opt_39 = define(/import)			; MPNC-CAT new HDF5 file read as a list-mode
		opt_39.name =		'pnc_cat_hdf_evt'	; unique name of import
		opt_39.title =		'Extract E,X,Y from PNC_CAT HDF5 file as list-mode'
...
function pnc_cat_hdf_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'pnc_cat_hdf_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"pnc_cat_hdf_DEVICE" --> pnc_cat_hdf_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'PNC_CAT_HDF_DEVICE', $	; unique name for this device object
		title = 'PNC-CAT image data HDF5 file', $
...
pro pnc_cat_hdf_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('pnc_cat_hdf_DEVICE')

COMPILE_OPT STRICTARR

maia = { PNC_CAT_HDF_DEVICE,  $

;
;  Deadtime Detector Spectrum plugin routine
;  -----------------------------
;
...
pro Deadtime_Detector_spectrum_plugin, p, ii, marks, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = '* Deadtime by Detector Plugin'			; return the menu title for this plugin
	...
	h = 100. * *(*p[0]).deadtime_det
	
	window, 0
	!p.title = 'Dead-time Losses per Detector'
	!x.title = 'Detector Channel'
	!y.title = 'Dead-time Fraction (%)'
	plot, h, /nodata
	oplot, h, psym=10, color=spec_colour('green')

;	history = 'Deadtime Detector Histogram plugin'		; a history record for this plugin

	return

bad_hist:
	warning,'Deadtime_Detector_spectrum_plugin','No Deadtime detector histogram found.'
	return
end

;
;  raw dwell_map Image plugin routine
;  -----------------------------
;
...
pro raw_dwell_map_image_plugin, p, i, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = 'Dwell map Image Plugin'		; return the menu title for this plugin
	return
endif
...
if (*p).has_dwell then begin
	dwell_map = *(*p).dwell_map
	name = 'Dwell'

	q = where( (dwell_map) ge 0.5*max(dwell_map), nq)
	q2 = where( ((dwell_map) lt 0.5*max(dwell_map)) and ((dwell_map) gt 0.), nq2)
	q3 = where( (dwell_map) eq 0., nq3)
	print, 'average dwell_map = ', mean(dwell_map)
	...
	top = max(dwell_map)
	binsiz = 0.01 * top
	h = histogram( dwell_map, min=0., max=top, omin=omin,omax=omax, binsize=binsiz, locations=x)
	x = x+binsiz/2.

	!p.color = spec_colour('black')
	!p.background = spec_colour('white')
	!p.title = 'Dwell time distribution'
	!x.title = 'Dwell (ms) per pixel'
	!y.title = 'Frequency'
	!p.charsize = 1.0	;1.2
	!p.charthick = 1.0
	!p.thick = 1.0
	...
	xyouts,0.90,0.87, 'Max dwell = '+str_tidy(omax,places=1)+' (ms)', /norm,align=1
...
;	No need to add the plugin name here; this is done by GeoPIXE.
	...
	history = 'dwell_map found'
endif else begin
	print,'Dwell map plugin: No "dwell_map" array found.'
	history = 'No dwell_map found'
endelse

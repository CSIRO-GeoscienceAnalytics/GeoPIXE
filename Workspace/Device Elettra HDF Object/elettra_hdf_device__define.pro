...
;
; name()				return name of device (e.g. "ELETTRA_HDF_DEVICE").
...
pro ELETTRA_HDF_DEVICE::cleanup
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'ELETTRA_HDF_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
function ELETTRA_HDF_DEVICE::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ELETTRA_HDF_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	if H5F_IS_HDF5(strip_file_ext(file)+'.h5') eq 0 then begin
		warning,'ELETTRA_HDF_DEVICE::get_header_info','Not a valid HDF5 file: '+strip_file_ext(file)+'.h5'
		self.header.error = 1
		return, self.header
	endif
...
pro ELETTRA_HDF_DEVICE::flux_scan, unit, evt_file, PV_list, IC_name, IC_val, IC_vunit, dwell=dwell, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ELETTRA_HDF_DEVICE::flux_scan',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
bad_io:
	warning,'ELETTRA_HDF_DEVICE::flux_scan','HDF file I/O error.'
	error = 1
	return
end
...
function ELETTRA_HDF_DEVICE::read_setup, unit, xrange, yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ELETTRA_HDF_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
;;	This will take up a lot of memory (approaching 1 Gb) for individual detectors.
 		...
 		if s gt 300 then begin
 			warning,'ELETTRA_HDF_DEVICE::read_setup','array in memory will be ',s,'Mbytes in size ...'
 		endif

    hs_pixe_read_line = 0
    hdf5_spectra = H5D_READ(spectra_id)
    ;hdf_sd_getdata, sdsSPECTRA, hdf5_spectra, start=[0,0], count=[hs_pixe_x_range*hs_pixe_y_range,2048]
...
function ELETTRA_HDF_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ELETTRA_HDF_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
;     hdf_sd_getdata, sdsSPECTRA, hdf5_spectra, start=[i_buffer,0], count=[hs_pixe_x_range,2048]
			hs_pixe_read_line = 0
		endif
...
function ELETTRA_HDF_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ELETTRA_HDF_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'elettra_hdf_evt': begin						; 41
		  ...
		  p2 = ptrarr(1)
		  p2[0] = ptr_new(spec)
			;warning,'ELETTRA_HDF_DEVICE::import_spec',['"elettra_hdf_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p2
end
...
function ELETTRA_HDF_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_41 = define(/import)			; HS_PIXE HDF image file
		opt_41.name =		'elettra_hdf_evt'		; unique name of import
		opt_41.title =		'Extract from Elettra image HDF file'
		opt_41.in_ext =		'.h5'		; input file extension
		opt_41.request =	'Select Elettra HDF file to scan for all spectra'
...
function ELETTRA_HDF_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ELETTRA_HDF_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"ELETTRA_HDF_DEVICE" --> ELETTRA_HDF_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'ELETTRA_HDF_DEVICE', $	; unique name for this device object
		title = 'Elettra image data file', $
...
pro ELETTRA_HDF_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('ELETTRA_HDF_DEVICE')

COMPILE_OPT STRICTARR

maia = {ELETTRA_HDF_DEVICE,  $

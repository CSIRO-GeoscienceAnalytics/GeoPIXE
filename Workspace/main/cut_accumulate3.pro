function cut_accumulate3, x,y,e,n, image,nx,ny, error,nxe,nye, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'Cut_accumulate3',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'cut_accumulate3'), cdecl=geolib_cdecl(), $
			x,y,e,long(n), image,long(nx),long(ny), $
			error,long(nxe),long(nye), $
			cut,type,dleft,dright,long(nel), multiple, long(stim_mean), $
			image_count, error_count, value=value )
return, err

bad:
	gprint,' cut_accumulate3: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

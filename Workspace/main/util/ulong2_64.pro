function ulong2_64, s, error=err

; Covert 's' into a ULONG64 if it is a valid integer string
;
; Chris Ryan (CSIRO), 2018, based on long2

err = 0
n = n_elements(s)
if n lt 1 then return, 0
if size(s,/tname) ne 'STRING' then return, ulong64(s)
...
q = where( ok eq 1, nq)
if nq gt 0 then x[q] = ulong64(s[q])

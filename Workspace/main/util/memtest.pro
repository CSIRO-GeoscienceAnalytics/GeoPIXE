...
pro memtest
  ...
  memBlockSizes = ulon64arr(maxIterations)
  s = strarr(20)
  
  for i=0L, maxIterations-1 do begin
  ; Error handler
    catch, err

    ; Specifically designed for "Failure to allocate memory..." error
  ...
  ; Print the current allocated block size and the running total, in Mb
  	t = string( i + 1, ishft(currentBlockSize, -20), ishft(ulong64(total(memBlockSizes)), -20), $
  			format='(%"Memory block #%3d: %5d Mb (total: %6d Mb)")')
    print, t
    s[i] = t
  endfor

  warning, 'memtest', s, /info
  wait,1
  ptr_free, memPtrs
end

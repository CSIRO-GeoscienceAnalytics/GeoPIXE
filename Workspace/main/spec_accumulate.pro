function spec_accumulate, e,x,y,n, spec,ns,found, mask,nx,ny,nq, $
								multiple=multiple
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'Spec_accumulate',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'spec_accumulate'), cdecl=geolib_cdecl(), $
			e,x,y,long(n), spec,long(ns),found, $
			mask,long(nx),long(ny),long(nq), multiple, value=value )

return, err

bad:
	print,' spec_accumulate: Error - bad type for argument.'
	err = 1L
	return, err
end

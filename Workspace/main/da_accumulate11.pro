function da_accumulate11, x,y,e,pu,veto,z,n,good, image,nx,ny, nnpu,nn, image_error,nxe,nye, $
						nel, matrix, size, n_planes, n_energy, multiple=multiple
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'da_accumulate11',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'da_accumulate11'), cdecl=geolib_cdecl(), $
		x,y,e,pu,veto,z,long(n),long(good), image,long(nx),long(ny), nnpu,nn, image_error,long(nxe),long(nye), $
		long(nel), matrix, long(size), long(n_planes), long(n_energy), multiple, value=value )

return, err

bad:
	gprint,' da_accumulate11: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

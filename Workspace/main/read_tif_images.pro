function read_tif_images, file, header=header, error=error
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'read_tif_images',['IDL run-time error caught.', '', $
...
bad_extra:
	print,'read_tif_images: no extra data'
	goto, finish
bad_io:
	print,'read_tif_images: I/O error'
	goto, error
bad_file:
	print,'read_tif_images: no file name supplied'
	goto, usage
bad_version:
	print,'read_tif_images: bad version number'
	goto, error
bad_user:
	print,'read_tif_images: bad user mode'
	goto, error

usage:
	print,'read_tif_images: Usage: p = read_tif_images(file)'

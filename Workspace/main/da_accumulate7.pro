function da_accumulate7, x,y,e,pu,veto,n,good, image,nx,ny,nel2, nnpu,nn, image_error,nxe,nye, $
						nel, matrix, size, table, multiple=multiple, q=q, compress=compress
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'da_accumulate7',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'da_accumulate7'), cdecl=geolib_cdecl(), $
		x,y,e,pu,veto,long(n),long(good), image,long(nx),long(ny),long(nel2), nnpu,nn, image_error,long(nxe),long(nye), $
		long(nel), matrix, long(size), table, q, net, ncomp, nxt, nyt, long(compress>1), multiple, value=value )

return, err

bad:
	gprint,' da_accumulate7: Error - bad type for argument: '+sb
	err = 1L
	return, err
bad_array:
	gprint,' da_accumulate7: Error - array dimensions for argument: '+sb
	err = 1L
	return, err
end

function da_accumulate4, x,y,e,pu,n, image,nx,ny, nnpu,nn, image_error,nxe,nye, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'da_accumulate4',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'da_accumulate4'), cdecl=geolib_cdecl(), $
		x,y,e,pu,long(n), image,long(nx),long(ny), nnpu,nn, image_error,long(nxe),long(nye), $
		long(nel), matrix, long(size), multiple, value=value )

return, err

bad:
	gprint,' da_accumulate4: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

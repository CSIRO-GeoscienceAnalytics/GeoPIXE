	function relative_intensity_xrf, zi, ni
;
...
;
	...
	if nz eq 1 then begin
		...
		print,'relative_intensity_xrf: Z,N can only both be vectors if they have same length.'
		e = fltarr(nt)
		goto, done
	endif
	e = fltarr(nt)
	...
	q = where( ((z ge 1) and (z lt n_elements(xrelint[0,*]))) and $
				((n ge 1) and (n lt n_elements(xrelint[*,0]))) )

pro da_evt_tomo, filei, files=files, matrix=matrix_file, xrange=xrange, yrange=yrange, zrange=zrange, charge=charge, $
          cal_a=cal_a, cal_b=cal_b, xcompress=xcompress, ycompress=ycompress, zcompress=zcompress, $
...
;
;	translate_file	file for table (zrange long) of angles for each Z(rotation) bin.
;	
...
gprint,level=2, output=cluster_debug, 'da_evt_tomo: start time = ',systime()

;cluster = 1
;cluster_total = 3										; for debugging
;cluster_index = 2
;openw, cluster_debug, 'da_evt_tomo_debug.txt', /get_lun
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       if cluster then begin
       		worker_error, stringify( {command:'da_evt_tomo', error:s2 })
       		gprint,level=2, output=cluster_debug, 'da_evt_tomo', s2
       endif else begin
	       warning,'da_evt_tomo', s2, /error
       endelse
       MESSAGE, /RESET
       return
    endif
...
;    gprint, output=cluster_debug, 'da_evt_tomo: missing arguments'
;    return
;endif
...
if use_ylut eq 0 then begin
	if cluster then warning,'da_evt_tomo','Veto "cluster" mode and YLUT for the moment.'
	cluster = 0
	build_ylut = 0
...
matrix = read_da( matrix_file, error=err)
...
; Maia 'Correct Y Encoder' mode. These are built during da_evt_tomo processing below.
...
image = fltarr(xrange3,yrange3, matrix.n_el, zrange3 )
image_error = fltarr(xrange3e,yrange3e, matrix.n_el, zrange3e)
...
if random_subset then warning,'da_evt_tomo','Random subset of "'+str_tidy(accept_fraction)+'" selected.'
...
;		(for veto) or in 'read_buffer' (for ste, t, multiple).
	...
	if j eq 18 then begin
		...
		read_buffer, obj, 1, x1,y1,e, channel,n, xcompress,ycompress, z1=z1, zcompress=zcompress, $
		...
		if do_progress then begin
			...
			if iprogress ge nprogress then begin
				...
				if dt gt 0.2*ft then begin
					...
					gprint, level=2, output=cluster_debug,'da_evt_tomo: Extend progress period to ',nprogress
				endif
				iprogress = 0L
				last_time = time
			endif
...
;      'image' at coords 'x2,y2', for each element.
			...
			err = da_accumulate_tomo( x1,y1,z1,col,pu,veto, n,count2, image,xrange3,yrange3,zrange3, nnpu,nn, $
							image_error,xrange3e,yrange3e,zrange3e, matrix.n_el, da_matrix,matrix.size, $
							multiple=multiple)
			if err ne 0 then begin
				gprint,level=2, output=cluster_debug,'da_evt_tomo: error (',err,') return from da_accumulate3'
				goto, finish
			endif
		endif
		if events gt 0 then if processed gt events then begin
			gprint,level=2, output=cluster_debug,'da_evt_tomo: requested event count exceeded; stop.'
			goto, finish
		endif
...
next:
	j = j+1
	first = 0
	gprint, level=1, output=cluster_debug,'da_evt_tomo: next file ...'
	if j lt nj then goto, loop_file

finish:
	gprint,level=2, output=cluster_debug,'da_evt_tomo: finished.'
...
nxye = long(xrange3) * long(yrange3) * matrix.n_el
ninf = 0L
for j=0L,zrange3-1 do begin
	for i=0L,matrix.n_el-1 do begin
		q = where(finite(image[*,*,i,j]) eq 0, nq)
		if nq gt 0 then begin
			image[q + nxy*i +nxye*j] = 0.0
			ninf = ninf + nq
		endif
	endfor
endfor
...
gprint,level=2, output=cluster_debug, 'da_evt_tomo: write image file stub - ' + output
...
;img.pz_coords = ptr_new( translate_energy)
;img.energies_file = translate_file

z_coords = findgen(zrange3) * scanz/float(zrange3-1)
img.pz_coords = ptr_new(z_coords)
...
; For tomo define charge, total flux as per energy plane ...
...
;		2	Dead time is not implemented (yet) or disabled
	...
	if mode eq 2 then begin
		...
		if nq1 gt 0 then begin
			gprint,level=2, output=cluster_debug,'da_evt_tomo: normalize dead-fraction to weight'
	
			dead_fraction[q1] = dead_fraction[q1] / weight[q1]
			if mode eq 1 then begin
				gprint,level=2, output=cluster_debug,'da_evt_tomo: Correct from OCR to ICR weights (mode=1).'
...
if (err_dwell eq 0) and (n_elements(dwell) gt 2) then begin
	...
	if nq gt 0 then begin
		gprint,level=2, output=cluster_debug,'da_evt_tomo: Dwell-time, ave: = ', mean(dwell)

		if mode le -1 then begin
			dead_fraction[q] = dead_fraction[q] / dwell[q]
		endif
		gprint,level=2, output=cluster_debug,'da_evt_tomo: dead-fraction, ave: = ', mean(dead_fraction)

		count_rate_map[q] = 1000. * float(nn[q]) / dwell[q]
		gprint,level=2, output=cluster_debug,'da_evt_tomo: count-rate, ave: = ', mean(count_rate_map)
		img.has_rates = 1
		...
		gprint,level=2, output=cluster_debug,'da_evt_tomo: Dwell-time, zero !'
	endelse
endif
...
if n_elements(dead_fraction) gt 1 then begin
	if n_elements(flux[*,*,0]) eq n_elements(dead_fraction) then begin
    	...
    	if tdead gt 0.001 then begin
			...
			if nq gt 0 then begin
				gprint,level=2, output=cluster_debug,'da_evt_tomo: Extreme dead-time fraction for '+str_tidy(nq)+' pixels. Clip these to 0.5.'
...
if do_pileup and (n_elements(dead_fraction) gt 1) then begin
	if (n_elements(flux[*,*,0]) eq n_elements(pileup_loss_map)) then begin
		...
		if nq gt 0 then begin
			...
			if nq gt 0 then begin
				gprint,level=2, output=cluster_debug,'da_evt_tomo','Extreme pileup-loss fraction for '+str_tidy(nq)+' pixels. Clip these to 0.9.'
				pileup_loss_map[q] = 0.95
			endif
		endif	
	   	gprint,level=2, output=cluster_debug,'da_evt_tomo: pile-up losses, ave: = ', mean(pileup_loss_map)

		if (n_elements(dead_fraction) gt 1) then begin
			gprint,level=2, output=cluster_debug,'da_evt_tomo: dead-time losses, ave: = ', mean(dead_fraction)
			if (n_elements(flux[*,*,0]) eq n_elements(dead_fraction)) then begin
...
if n_elements(dead_fraction) gt 1 then begin
	if n_elements(flux[*,*,0]) eq n_elements(dead_fraction) then begin
		tdead = total(dead_fraction)/n_elements(dead_fraction)
    	if tdead gt 0.001 then begin
			gprint,level=2, output=cluster_debug,'da_evt_tomo: correct flux array for DT+PU losses, ave: = ', mean(dead_fraction)

	    	for i=0,zrange3-1 do flux[*,*,i] = flux[*,*,i] * (1. - dead_fraction)
		endif
		img.has_dead = 1
...
img.el = matrix.el[0]
img.ixanes = 0
img.stack_type = 1							; tomo type
output1 = strip_file_ext(output) + '-tomo-' + img.el + '.' + extract_extension(output)
...
; preserve the average. Flatten all elements the same.

if n_elements(flux) gt 1 then begin
	aps_count_to_charge = flux_ic.conversion
	if flatten then gprint,level=2, output=cluster_debug,'da_evt_tomo: Correct images for flux ...'
...
;gprint,level=2, output=cluster_debug,'da_evt_tomo: pimg struct:'
;pointer_display, unit=cluster_debug, pimg

if dummy_write then begin
	gprint,level=2, output=cluster_debug,'da_evt_tomo: Dummy save of images to file ',output
endif else begin
	gprint,level=2, output=cluster_debug,'da_evt_tomo: Write stacks ...'
	timage = (*pimg).image 
	terror = (*pimg).error

	for i=0,matrix.n_el-1 do begin
		(*pimg).el = matrix.el[i]
		(*pimg).ixanes = i
		(*pimg).stack_type = 1	
		output1 = strip_file_ext(output) + '-tomo-' + (*pimg).el + '.' + extract_extension(output)
		(*pimg).file = output1

		(*pimg).image = ptr_new( reform( (*timage)[*,*,i,*]) )
		(*pimg).error = ptr_new( reform( (*terror)[*,*,i,*]) )

		write_geopixe_image, pimg, output1, /no_display, cluster=cluster, error=err
		if err then gprint,level=2, output=cluster_debug,'da_evt_tomo: Error writing stack: '+output1		
		
		if (i lt matrix.n_el-1) then begin
			ptr_free, (*pimg).image
			ptr_free, (*pimg).error
		endif
	endfor
	...
	output1 = strip_file_ext(output) + '-tomo-' + (*pimg).el + '.' + extract_extension(output)
	(*pimg).file = output1
	(*pimg).image = ptr_new( flux )
	write_geopixe_image, pimg, output1, /no_display, cluster=cluster, error=err
	if err then gprint,level=2, output=cluster_debug,'da_evt_tomo: Error writing flux stack: '+output1		

	gprint,level=2, output=cluster_debug,'da_evt_tomo: Stack writes completed.'
endelse
...
if arg_present(images) eq 0 then begin
	...
	output1 = strip_file_ext(output) + '-tomo-' + (*pimg).el + '.' + extract_extension(output)
	(*pimg).file = output1
	*(*pimg).image = reform( (*timage)[*,*,0,*] )
	*(*pimg).error = reform( (*terror)[*,*,0,*] )
    images = pimg
endelse
if ptr_valid(timage) then ptr_free, timage 
if ptr_valid(terror) then ptr_free, terror
cluster_result = output

cleanup:
	close, 1
	gprint,level=2, output=cluster_debug, 'da_evt_tomo: finish time = ',systime()
    if do_progress and (cluster eq 0) then progress, /ending, progress_tlb
	free_DA, matrix
    return

bad_table:
    warning, output=cluster_debug, 'da_evt_tomo', 'Missing or short energy Translation table.'
    goto, cleanup
bad_matrix:
    warning, output=cluster_debug, 'da_evt_tomo', 'Bad DA matrix.'
    goto, cleanup
bad_matrix2:
    warning, output=cluster_debug, 'da_evt_tomo', 'Matrix does not seem to be a tomo series.'
    goto, cleanup
bad_file:
    warning, output=cluster_debug, 'da_evt_tomo', 'EVT file not found.'
    goto, cleanup
bad_el:
    warning, output=cluster_debug, 'da_evt_tomo,', 'No element selected, or not found in DA matrix.'
    goto, cleanup
bad_obj:
	warning, output=cluster_debug,'da_evt_tomo', 'Bad device object for: '+device
    goto, cleanup
bad_ylut_ymode:
    warning, output=cluster_debug, 'da_evt_tomo', ['Cannot use "cluster" mode first with "Correct Y Encoder" mode.', $
    				'Do a normal (non-cluster) sort first to build a valid YLUT file.']
    goto, cleanup
bad_ylut_build:
    warning, output=cluster_debug, 'da_evt_tomo', 'Error building Y Lookup Table.'
    goto, cleanup
bad_ylut_get:
    warning, output=cluster_debug, 'da_evt_tomo', 'Error reading Y Lookup Table.'
    goto, cleanup
bad_cluster:
;	warning, output=cluster_debug, 'da_evt_tomo', 'No EVT files for this node.'
    goto, cleanup
bad_z1:
	warning, output=cluster_debug, 'da_evt_tomo', 'Device does not support third axis.'
    goto, cleanup
end

pro cube_evt, filei, files=files, xrange=xrange, yrange=yrange, charge=charge, $
		  ...
		  facility=facility, endstation=endstation, pv_list=pv_list, flatten=flatten, $
          
          images=images, xorigin=xorigin, yorigin=yorigin, xoffset=xoffset, $
		  yoffset=yoffset, x_sub_range=x_sub_range, y_sub_range=y_sub_range, $ 
...
gprint,level=2, output=cluster_debug, 'cube_evt: start time = ',systime()

;cluster = 1
;cluster_total = 5										; for debugging
;cluster_index = 2										; but missing 'worker_progress' call ...
;openw, cluster_debug, 'cube_evt_debug.txt', /get_lun
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       if cluster then begin
       		worker_error, stringify( {command:'cube_evt', error:s2 })
       		gprint,level=2, output=cluster_debug, 'cube_evt', s2
       endif else begin
	       warning,'cube_evt', s2, /error
       endelse
       MESSAGE, /RESET
       return
    endif
...
;    gprint, output=cluster_debug, 'cube_evt: missing arguments'
;    return
;endif
...
; Maia 'Correct Y Encoder' mode. These are built during cube_evt processing below.
...
gprint, output=cluster_debug,'xrange3,yrange3=',xrange3,yrange3
toc, lun=cluster_debug

; Compressed E scale based on sqrt() and compressed E range 2-35 keV (sqrted) into range of 200 bins.

zrange = 200

image = fltarr(xrange3,yrange3,zrange + n_attributes)
...
if random_subset then warning,'cube_evt','Random subset of "'+str_tidy(accept_fraction)+'" selected.'
...
;		Compressed E scale based on sqrt() and compressed E range 2-35 keV (sqrted) into range of 200 bins.
;		Translate from energy E to compressed (sqrt) E scale in Z ...

		elow = 2.
		ehigh = 35.
		xhigh = zrange-1
		scale = float(xhigh) / (sqrt(ehigh) - sqrt(elow))
		z1 = uint(scale * (sqrt(energy) - sqrt(elow)))

;     Reject those events outside the pixel range
;     'count2' is the total remaining good events.

		q1 = where( (veto eq 0) and (((x1 lt 0) or (x1 ge xrange3)) or  $
										((y1 lt 0) or (y1 ge yrange3)) or $
										((z1 lt 0) or (z1 ge zrange))), nq1)
		clipped = clipped + nq1
		...
		if do_progress then begin
			...
			if iprogress ge nprogress then begin
				...
				if dt gt 0.2*ft then begin
					...
					gprint, level=2, output=cluster_debug,'cube_evt: Extend progress period to ',nprogress
				endif
				iprogress = 0L
				last_time = time
			endif
...
;			'image' at coords 'x2,y2', for each element.
			...
			err = da_accumulate_cube( x1,y1,z1,pu,veto, n,count2, image,xrange3,yrange3,zrange+n_attributes, nnpu,nn, multiple=multiple)
			if err ne 0 then begin
				gprint,level=2, output=cluster_debug,'cube_evt: error (',err,') return from da_accumulate'
				goto, finish
			endif
		endif
		if events gt 0 then if processed gt events then begin
			gprint,level=2, output=cluster_debug,'cube_evt: requested event count exceeded; stop.'
			goto, finish
		endif
...
next:
	j = j+1
	first = 0
	gprint, level=1, output=cluster_debug,'cube_evt: next file ...'
	if j lt nj then goto, loop_file

finish:
	gprint,level=2, output=cluster_debug,'cube_evt: finished.'
...
gprint,level=2, output=cluster_debug, 'cube_evt: write image file - ' + output
...
tz = findgen(zrange)
zlabel = str_tidy( ((tz/scale) + sqrt(elow))^2, len=5)

img.n_el = zrange
img.el = ptr_new( zlabel)
...
img.type = 2						; 0=ppm.uC data, 1=mineral fractions, 2=counts, 3=energy
									; units = ['keV','MeV','MeV']
img.mode = 4						; 0=DA, 1=CUTs, 2=CUT energy (STIM), 3=MPDA, 4=cube
...
;		2	Dead time is not implemented (yet) or disabled
	...
	if mode eq 2 then begin
		...
		if nq1 gt 0 then begin
			gprint,level=2, output=cluster_debug,'cube_evt: normalize dead-fraction to weight'
	
			dead_fraction[q1] = dead_fraction[q1] / weight[q1]
			if mode eq 1 then begin
				gprint,level=2, output=cluster_debug,'cube_evt: Correct from OCR to ICR weights (mode=1).'
...
if (err_dwell eq 0) and (n_elements(dwell) gt 2) then begin
	...
	if nq gt 0 then begin
		...
		gprint,level=2, output=cluster_debug,'cube_evt: Dwell-time, ave: = ', mean_dwell, ' ms'

		if mode le -1 then begin
			dead_fraction[q] = dead_fraction[q] / dwell[q]
		endif
		gprint,level=2, output=cluster_debug,'cube_evt: dead-fraction, ave: = ', mean(dead_fraction[q])

		count_rate_map[q] = 1000. * float(nn[q]) / dwell[q]
		gprint,level=2, output=cluster_debug,'cube_evt: count-rate, ave: = ', mean(count_rate_map[q])
		img.has_rates = 1
		...
		gprint,level=2, output=cluster_debug,'cube_evt: Dwell-time, zero !'
	endelse
endif
...
if n_elements(dead_fraction) gt 1 then begin
	if n_elements(flux[*,*,0]) eq n_elements(dead_fraction) then begin
    	...
    	if tdead gt 0.001 then begin
			...
			if nq gt 0 then begin
				gprint,level=2, output=cluster_debug,'cube_evt: Extreme dead-time fraction for '+str_tidy(nq)+' pixels. Clip these to 0.95.'
...
if do_pileup and (n_elements(dead_fraction) gt 1) then begin
	if (n_elements(flux[*,*,0]) eq n_elements(pileup_loss_map)) then begin
		...
		if nq gt 0 then begin
			...
			if nq gt 0 then begin
				gprint,level=2, output=cluster_debug,'cube_evt','Extreme pileup-loss fraction for '+str_tidy(nq)+' pixels. Clip these to 0.95.'
				pileup_loss_map[q] = 0.95
			endif
		endif	
	   	gprint,level=2, output=cluster_debug,'cube_evt: pile-up losses, ave: = ', mean(pileup_loss_map)

		if (n_elements(dead_fraction) gt 1) then begin
			gprint,level=2, output=cluster_debug,'cube_evt: dead-time losses, ave: = ', mean(dead_fraction)
			if (n_elements(flux[*,*,0]) eq n_elements(dead_fraction)) then begin
...
if n_elements(dead_fraction) gt 1 then begin
	if n_elements(flux[*,*,0]) eq n_elements(dead_fraction) then begin
    	...
    	if tdead gt 0.00001 then begin
			gprint,level=2, output=cluster_debug,'cube_evt: correct flux array for DT+PU losses, ave: = ', mean(dead_fraction)
...
gprint,level=2, output=cluster_debug,'cube_evt: n_el, n_attributes =', img.n_el, n_attributes
...
if n_elements(flux) gt 1 then begin
	...
	if flatten then gprint,level=2, output=cluster_debug,'cube_evt: Correct images for flux ...'
...
gprint,level=2, output=cluster_debug,'cube_evt: pimg struct:'
;pointer_display, unit=cluster_debug, pimg

if dummy_write then begin
	gprint,level=2, output=cluster_debug,'cube_evt: Dummy save of images to file ',output
endif else begin
	gprint,level=2, output=cluster_debug,'cube_evt: Write images ...'
	write_geopixe_image, pimg, output, /no_display, cluster=cluster, error=err
	if err eq 0 then gprint,level=2, output=cluster_debug,'cube_evt: Image write completed.'
endelse
...
cleanup:
	gprint,level=2, output=cluster_debug, 'cube_evt: finish time = ',systime()
    if do_progress and (cluster eq 0) then progress, /ending, progress_tlb
	toc, lun=cluster_debug
    return

bad_file:
    warning, output=cluster_debug, 'cube_evt', 'EVT file not found.'
    goto, cleanup

bad_obj:
	warning, output=cluster_debug,'cube_evt', 'Bad device object for: '+device
    goto, cleanup

bad_ylut_ymode:
    warning, output=cluster_debug, 'cube_evt', ['Cannot use "cluster" mode first with "Correct Y Encoder" mode.', $
    				'Do a normal (non-cluster) sort first to build a valid YLUT file.']
    goto, cleanup

bad_ylut_build:
    warning, output=cluster_debug, 'cube_evt', 'Error building Y Lookup Table.'
    goto, cleanup

bad_ylut_get:
    warning, output=cluster_debug, 'cube_evt', 'Error reading Y Lookup Table.'
    goto, cleanup

bad_cluster:
	warning, output=cluster_debug, 'cube_evt', 'No EVT files for this node.'
    goto, cleanup
end

function cut_accumulate4, x,y,e,pu,n, image,nx,ny, nnpu,nn, error,nxe,nye, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'cut_accumulate4',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'cut_accumulate4'), cdecl=geolib_cdecl(), $
			x,y,e,pu,long(n), image,long(nx),long(ny), nnpu,nn, $
			error,long(nxe),long(nye), cut,type,dleft,dright,long(nel), multiple, long(stim_mean), $
			image_count, error_count, value=value )
return, err

bad:
	gprint,' cut_accumulate4: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

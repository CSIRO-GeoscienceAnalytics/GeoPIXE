...
;
;	throttle = throttle_select( group, tot)
;
...
pro throttle_select_event, Event
	...
	case uname of
		...
		'ok': begin
			...
			if fnumeric(s) then begin
				...
				warning,'throttle_select_event','Illegal character in Time.'
				error = 1
			endelse
			(*(*pstate).p).rate = (*(*pstate).p).total * (*(*pstate).p).detectors / (*(*pstate).p).time
			widget_control, (*pstate).max, get_value=s
			if fnumeric(s) then begin
				...
				warning,'throttle_select_event','Illegal character in Max rate.'
				error = 1
			endelse
			(*(*pstate).p).factor = (*(*pstate).p).rate / (*(*pstate).p).max
...
function throttle_select, group, tot
...
COMPILE_OPT STRICTARR
	...
	tlb = widget_base( /column, title='Enter Throttle Pars', /TLB_KILL_REQUEST_EVENTS, xoffset=xoffset, yoffset=yoffset, $
					group_leader=group, uname='throttle_select_TLB', /base_align_center, $
	...
	rate_id = widget_label( s3base, value='              ',scr_xsize=100)
	...
	p = ptr_new( {total:tot, time:0.0, max:2.e6, rate:0.0, factor:0.0, detectors:32, error:0 })
	...
	xmanager, 'throttle_select', tlb					;, /no_block

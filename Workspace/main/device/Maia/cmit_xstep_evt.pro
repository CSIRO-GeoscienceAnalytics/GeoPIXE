pro cmit_xstep_evt, filei, xstep_count, step_events=step_events, xrange=xrange, yrange=yrange, $
          ...
          progress=do_progress, device=devicei, ystep=ystep, group=group, throttle = throttlef
;
...
maia = 1              ; output in new Maia 32 format
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       warning,'cmit_xstep_evt',['IDL run-time error caught.', '', $
...
if ((step_toggle eq 1) and (n_params() lt 2)) or $
    ...
    print,'cmit_xstep_evt: missing arguments'
    return
endif
...
if step_toggle eq 1 then begin
    ...
    if step_events then begin
       if n_elements(step_station) lt 1 then step_station = channel[0]
    endif else begin
...
loop_file:
    ...
    if j eq 0 then begin
       if do_progress then begin
         progress, tlb=progress_tlb, title='Convert '+direction+' Step XY EVT file', $     ; put up a progress bar
		 		pars=['Events','Valid','Blocks','Bad XY',direction,'Clipped']
    ...
    while ~ EOF(1) do begin
       ...
       read_buffer, obj, 1, x1,y1,e, channel,n, xcompress,ycompress,  $
          ...
          total_bad_xy=bad_xy, total_processed=processed, step_count=xstep_count, $
          step_toggle=step_toggle, toggle_bit=toggle_bit, toggle_adc=step_station, $
          processed=count1, valid=good, ystep=ystep, multiple=multiple, $
...
;      Want NOT sum energy, but maximum of amp_fun for first pulse.
       ...
       if events gt 0 then if processed gt events then begin
         print,'cmit_xstep_evt: requested event count exceeded; stop.'
         goto, finish
       endif
...
finish:
    ...
    if n_elements(p) gt 0 then begin
       if do_progress then begin
         p.value = [processed,valid,i,bad_xy,t,clipped]
...
bad_channels:
    warning, 'CMIT_xstep_evt', 'Channel array does not match Cal_a.'
    close, 1
    close, 2
    return
bad_file:
    warning, 'CMIT_xstep_evt', 'EVT File not found.'
    return
bad_file2:
    warning, 'cmit_xstep_evt', 'XVT File not openned.'
    close, 1
    return
bad_obj:
	warning, 'cmit_xstep_evt', 'Bad device object for: '+device
    close, 1
    return
bad_write:
    close, 1
    close, 2
    warning, 'CMIT_xstep_evt', 'bad write.'
    return
end

function get_maia_details, p, error=err
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       warning,'get_maia_details',['IDL run-time error caught.', '', $
...
;	
	...
	id = {name:'', file:'', X:0L, Y:0L, Z:0L, noY:1, run:0L, segment:0L, version:0L }
	dwell = 0.0
	nxpixels = 0
	nypixels = 0
	nzpixels = 0
	    ...
	    if (*pr[i]).length gt 0 then begin
			...
			case (*pr[i]).tag of
				...
				28: begin										; identity 2
					...
					blogd_path = str[5]
					id.name = facility
					id.file = str_tidy(id.run) + '.' + str_tidy(id.segment)
					end
				26: begin										; monitor
					b = (*(*pr[i]).b)[0:min([n_bytes,(*pr[i]).length])-1]
					sc = string(b)
				...
				30: begin										; rexec1 info
					if n_elements( *(*pr[i]).b) lt 44 then goto, cont_mon
					b = (*(*pr[i]).b)[0:min([n_bytes,(*pr[i]).length])-1]
					b1 = byte(b,0,4)
					scan_type = b1[0]
					scan_major = b1[1]
				...
				8: begin										; ET data
					if n_elements( *(*pr[i]).b) lt 4 then goto, cont_mon
					b = (*(*pr[i]).b)[0:3]
					d = ulong(b,0,1)
				...
				34: begin										; event_1 (was ET4) data
					if n_elements( *(*pr[i]).b) lt 12 then goto, cont_mon
					b = (*(*pr[i]).b)[0:11]
					d = ulong(b,0,3)
					swap_bytes, d, /big_endian_data
				...
				42: begin										; scaninfo_1
					if n_elements( *(*pr[i]).b) lt 54 then goto, cont_mon
					b = (*(*pr[i]).b)[0:min([n_bytes,(*pr[i]).length])-1]
				...
				44: begin										; DA info
					if n_elements( *(*pr[i]).b) lt 16 then goto, cont_mon
					b = (*(*pr[i]).b)[0:min([n_bytes,(*pr[i]).length])-1]
						...
						case s1 of
							...
							'flux.chan[].unit': begin
								...
								if err eq 0 then begin
									...
									if (flux_chan0_unit ne '') and (IC_name eq '') then IC_name='Maia:scaler.FC0'
								endif
								end
							'flux.chan[].coeff': begin
								sv = strsplit( sarg, ' 	',/extract)
								val = float2(sv, error=err)
								if err eq 0 then begin
									nv = n_elements(sv)
							...
							'hermes[].ech[]': begin
								sv = strsplit( sarg, ' 	',/extract)
								val = fix2(sv, error=err)
								if err eq 0 then begin
									nv = n_elements(sv)
									n_detectors = nv
									cal[0:n_detectors-1].on = val[0:n_detectors-1] eq 0
								endif
								end
...
;									if (k_da ge 0) and (k_da lt nv) then begin			; not a reliable source
;										st = str_escape( sval[k_da],/strip)
;										t = unstringify( st, error=err, context='get_maia_details')
;										if tag_present('IC',t) then begin
;											if tag_present('pv',t.IC) then begin
;												if tag_present('name',t.IC.pv) then begin
;													if t.IC.pv.name ne 'none' then begin
;														if tag_present('val',t.IC.pv) and tag_present('unit',t.IC.pv) then begin
;															flux_chan0_name = t.IC.pv.name
;															flux_chan0_coeff = float2(t.IC.pv.val) * float2(t.IC.pv.unit) 
;															flux_chan0_unit = 'nA/V'
;															if (IC_name eq '') then IC_name='Maia:scaler.FC0'
;														endif
;													endif
;												endif
;											endif
...
;									endif
							...
							'da.info': begin
;								t = unstringify( str_escape(s2,/strip), context='get_maia_details')			; not a reliable source
;								if tag_present('IC',t) then begin
;									if tag_present('pv',t.IC) then begin
;										if tag_present('name',t.IC.pv) then begin
;											if t.IC.pv.name ne 'none' then begin
...
;								endif
							...
							'pileup.info': begin
								if (pileup_found eq 0) then begin
									...
									if simple eq 0 then begin							; old .info stringify structs
										t = unstringify( str_escape(s2,/strip), context='get_maia_details')
							...
							'throttle.info': begin
								if (throttle_found eq 0) then begin
									throttle_found = 1
									if simple eq 0 then begin							; old .info stringify structs
										t = unstringify( str_escape(s2,/strip), context='get_maia_details')
										if tag_present('on',t) then throttle_on = t.on
										if tag_present('file',t) then throttle_file = t.file
										if throttle_file eq '' then throttle_on=0
									endif else begin									; new .info (duplicate of metadata and var_val)
										if s2 ne '' then begin							; or older .info, pre-structs
				...
				47: begin										; scaninfo_2 (detect units later ...)
					...
					simple = (strmid( sinfo, 0,1) eq '{') ? 0 : 1 
					if simple eq 0 then begin
						t = unstringify(sinfo, context='get_maia_details')				; Petra script kluge
						if tag_present('energy',t) then if t.energy ne 0.0 then energy = t.energy
						if tag_present('sample',t) then if t.sample ne '' then sample = t.sample
						if tag_present('comment',t) then if t.comment ne '' then comment = t.comment
						if tag_present('region',t) then if t.region ne '' then grain = t.region
						if tag_present('IC_sensitivity',t) then if t.IC_sensitivity ne 0.0 then IC_sensitivity = t.IC_sensitivity
						if tag_present('IC_sensitivity0',t) then if t.IC_sensitivity0 ne 0.0 then begin
							IC_sensitivity = t.IC_sensitivity0
							IC_name = 'Maia:scaler.FC0'
						endif
					endif else begin
						str = strsplit( sinfo, string(10B), /extract)
						ns = n_elements(str)
						if ns ge 1 then begin											; XFM comment strings kluge
							comment = strjoin( str,', ')
				...
				55: begin										; metadata
					...
					ns = n_elements(str)
					for j=0,ns-1 do begin
						nb = locate(' ',str[j])					; space between key and value
						s1 = strmid( str[j], 0, nb)				; key
						nt = strlen(str[j])
						if nt ge nb+2 then begin
							s2 = strmid( str[j], nb+1)			; value string
						endif else s2 = ''
						case s1 of
							'sample_type': begin
								if s2 ne '' then sample_type = s2
								end
							'sample_name': begin
								if s2 ne '' then begin
									sample = s2
									if strupcase(sample) eq 'UDIMET' then begin
										if sample_type eq '' then sample_type='standard'
										if sample_serial eq '' then sample_serial='500'
										if energy lt 1. then energy=24100.
									endif
								endif
								end
							'sample_serial': begin
								if s2 ne '' then sample_serial = s2
								end
							'beam_energy': begin
								if s2 ne '' then begin
									energy = float2(s2)
									if (abs(energy-100.) lt 0.01) then begin
										energy = run_energy_lookup( id.run, error=error)
									endif
								endif
								end
							...
							'flux_chan0_name': begin
								if s2 ne '' then flux_chan0_name = s2
								end
							'flux_chan1_coeff': begin
								if s2 ne '' then flux_chan1_coeff = float2(s2)
								end
							'flux_chan1_unit': begin
								if s2 ne '' then flux_chan1_unit = s2
								end
							'flux_chan1_name': begin
								if s2 ne '' then flux_chan1_name = s2
								end
							'throttle_info': begin
							...
							'prefix': begin
								if s2 ne '' then prefix = s2							; facility name (e.g. "MM.Mel")
								end
							'endstation': begin
								if s2 ne '' then endstation = s2						; endstation (e.g. "1")
								end
							else:
						endcase
					endfor
...
cont_mon:
	...
	if ns ge 1 then begin
		...
		for i=0L,ns-1 do begin
			str = strsplit( monitor_string[i],' ', /extract)
			ns2 = n_elements(str)
			name[i] = str[0]
			if ns2 ge 4 then begin
;				if gnumeric(str[3]) eq 0 then begin
;					print,'debug here ...'
;				endif
				case str[2] of
					'DBR_STRING': begin
...
;	Did we see metadata values for IC ...
	...
	if (flux_chan0_coeff ne 0.0) and (flux_chan0_unit ne '') then begin
		IC_sensitivity0 = charge_sensitivity(flux_chan0_coeff, flux_chan0_unit, time=time)
		print,'Found metadata IC sensitivity (' + (time ? 'dwell' : 'FC0') + ') = ',IC_sensitivity0
...
;		How is this connected to the IC PV selection widget in IC struct?
	...
	if (flux_chan1_coeff ne 0.0) and (flux_chan1_unit ne '') then begin
		IC_sensitivity1 = charge_sensitivity(flux_chan1_coeff, flux_chan1_unit, time=time)
		print,'Found metadata IC sensitivity (FC1) = ',IC_sensitivity1
		
;		if IC_sensitivity eq 0.0 then begin
;			IC_sensitivity = IC_sensitivity1
;			IC_name = 'Maia:scaler.FC1' 
;		endif
	endif
	
;	This is a bit of a hack to detect sensitivity PVs. It's	a bit like code in Maia device
;	These Epics Monitor records are pretty useless, as they are logged every 10s and
;	hence are seldom 'seen' in ther header read, even at 2 MB.
;	
;	Ion chamber preamp sensitivity value

	q2 = where_tokens( ['A1sens_num'], name, nq2)
	
;	Ion chamber preamp sensitivity units

	q3 = where_tokens( ['A1sens_unit'], name, nq3)
	
	if (nq2 gt 0) and (nq3 gt 0) then begin
		IC_sensitivity3 = charge_sensitivity( float2(val[q2[0]]), val[q3[0]])
		print,'Found monitor/Epics IC sensitivity (A1sens_) = ',IC_sensitivity3

;		Only use the Epics Monitor value here if all other ones have failed ...
		
;		if IC_sensitivity eq 0.0 then IC_sensitivity = IC_sensitivity3
	endif
	
;	How to handle IC settings in future?
;	This block copied from device_specific.pro
	
	ns = n_elements(name)
	if ns gt 0 then begin
		for i=0L,ns-1 do begin
			j = locate( strlowcase(maia_energy_pv), strlowcase(name[i]))
			en = float2(val[i])
			if j ge 0 then if en gt 0.1 then energy = en
		endfor
	endif
	if energy gt 500. then energy = energy/1000.
	...
	details = {id:id, monitor:{time:monitor_time, name:name, val:val}, dwell:dwell, $
				...
				facility:prefix, endstation:endstation, cal:cal }
	err = 0
	return, details
end

function read_maia_segments, segments, old=old, n_buffer=n_buffer, state=state, $
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       warning,'read_maia_segments',['IDL run-time error caught.', '', $
...
if n_elements(n_buffer) lt 1 then n_buffer = 2000000LL
if n_elements(state) lt 1 then state = {x:0, y:0, enable:replicate(1B,96), time:0 }
...
if n_elements(unit) ge 1 then begin
	...
	if ns gt 1 then	begin
		progress, tlb=progress_tlb, title='Read Maia Segments'
		do_progress = 1
...
;				print,'read_maia_segments: unit=',lun,' segment=',segments[i]
		...
		pr2 = read_maia( lun, n_buffer=n_buffer, n_actual=n_actual, skip=skip, $
					select=select, state=state, progressbar=(not do_progress) and (not veto_progress))
;		pr2 = read_maia( lun, n_buffer=n_buffer, n_actual=n_actual, skip=skip, state=state)
...
bad_open:
	warning,'read_maia_segments','error opening segment file(s): '+segments[i]
	goto, done
bad_io:
	warning,'read_maia_segments','error reading segment file(s): '+segments[i<(ns-1)]
	goto, done
end
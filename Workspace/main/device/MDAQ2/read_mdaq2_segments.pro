function read_mdaq2_segments, segments, n_buffer=n_buffer, state=state, $
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       warning,'read_mdaq2_segments',['IDL run-time error caught.', '', $
...
if n_elements(unit) ge 1 then begin
	...
	if ns gt 1 then	begin
		progress, tlb=progress_tlb, title='Read mdaq2 Segments'
		do_progress = 1
	...
	for i=0L,ns-1 do begin
		...
		pr2 = read_mdaq2( lun, n_buffer=n_buffer, n_actual=n_actual, skip=skip, $
...
;	print, 'Read_mdaq2_segments: Total bytes read = ',nt
	if do_progress then progress, /complete, progress_tlb, 'Read mdaq2 Segments completed.'

done:
	if by_unit eq 0 then begin
;		print,'read_mdaq2_segments: close unit=',lun
		close_file, lun
	endif else begin
		point_lun, lun, 0
	endelse
...
bad_open:
	warning,'read_mdaq2_segments','error opening segment file(s): '+segments[i]
	goto, done
bad_io:
	warning,'read_mdaq2_segments','error reading segment file(s): '+segments[i<(ns-1)]
	goto, done
end
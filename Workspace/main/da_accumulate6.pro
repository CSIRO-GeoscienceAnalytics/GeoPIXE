function da_accumulate6, x,y,e,pu,veto,n,good, image,nx,ny, nnpu,nn, image_error,nxe,nye, $
						nel, matrix, size, phase, ncomp, multiple=multiple
;
; Version of da_accumulate5 for multi-phase DA matrix
; 
;  This assumes image:		fltarr(nx,ny,nel)
;			image_error:	fltarr(nxe,nye,nel)
;				nnpu:		lonarr(nxe,nye)
;				nn:			lonarr(nxe,nye)
;				matrix:		fltarr(size,nel,ncomp)			; multiphase DA matrices
;				phase:		fltarr(nx,ny,ncomp)				; phase maps
;				x,y,e,pu:	uintarr(n)
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'da_accumulate6',['IDL run-time error caught.', '', $
...
if size(phase,/tname) ne 'FLOAT' then begin sb='phase' & goto, bad & endif
if size(multiple,/tname) ne 'LONG' then begin sb='multiple' & goto, bad & endif

err = call_external( geopixe_library(), geolib_name( 'da_accumulate6'), cdecl=geolib_cdecl(), $
		x,y,e,pu,veto,long(n),long(good), image,long(nx),long(ny), nnpu,nn, image_error,long(nxe),long(nye), $
		long(nel), matrix, long(size), phase, long(ncomp), multiple, value=value )

return, err

bad:
	gprint,' da_accumulate6: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

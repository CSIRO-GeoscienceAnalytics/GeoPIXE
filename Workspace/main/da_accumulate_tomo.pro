function da_accumulate_tomo, x,y,z,e,pu,veto,n,good, image,nx,ny,nz, nnpu,nn, image_error,nxe,nye,nze, $
						nel, matrix, size, multiple=multiple
;
;  This assumes image is:	fltarr(nx,ny,nz,nel)
;			image_error:	fltarr(nxe,nye,nz,nel)
;				nnpu:		lonarr(nxe,nye)
;				nn:			lonarr(nxe,nye)
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'da_accumulate_tomo',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'da_accumulate_tomo'), cdecl=geolib_cdecl(), $
...
bad:
	gprint,' da_accumulate_tomo: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

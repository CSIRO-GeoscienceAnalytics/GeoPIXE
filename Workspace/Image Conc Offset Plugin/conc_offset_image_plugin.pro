;
;  conc_offset Image plugin routine
;  -----------------------------
;
...
pro conc_offset_image_plugin, p, i, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = 'Conc Offset Image Plugin'			; return the menu title for this plugin
	return
endif
...
offset = 100.								; offset in ppm
charge_per_pixel = charge / (sx*sy)


(*pimg)[*,*,i] = old_img + charge_per_pixel*offset		; write back the modified image data

; A simple history record for this plugin.
; No need to add the plugin name here; this is done by GeoPIXE.

history = 'Conc Offset by = '+string(offset)+' ppm'

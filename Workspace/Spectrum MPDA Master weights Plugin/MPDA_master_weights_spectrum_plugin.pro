;
;  Create a master weights spectrum to use with MPDA
;  -----------------------------
;
...
pro MPDA_master_weights_spectrum_plugin, p, ii, marks, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
    title = '* MPDA Master Weights Plugin'         ; return the menu title for this plugin
    return
endif

i = 0
n = n_elements(p)						; number of phase regions
if n le 1 then begin
	warning,'MPDA_master_weights_spectrum_plugin','Only one spectrum found.'
	return
endif
...
;	Here we limit low stats spectra in a funny way. 
;	May be a more direct way to achieve this ...

for j=0,n-1 do begin
	norm[j] = norm[j] > max(norm)/2000.	; was 100

	top = top > (*(*p[j]).data) / norm[j]
endfor
top = top * max(norm)

(*pspec) = top							; write back the modified spectrum data, over #0
...
history = 'MPDA Master Weights'			; a history record for this plugin

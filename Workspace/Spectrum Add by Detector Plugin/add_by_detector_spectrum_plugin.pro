...
pro add_by_detector_spectrum_plugin, p, ii, marks, title=title, history=history
...
if arg_present(title) then begin
    title = '* Add by Detector Plugin'          ; return the menu title for this plugin
    return
endif

path = extract_path( (*p[0]).file)
F = file_requester(filter = '*.spec', title='Load SPEC file to add', path=path, $	
...
pp1 = ptr_new(p, /no_copy)
spectrum_load_spectrum_increment, pp1, pp2, /free
									
history = 'Add by Detector, File='+F[0]			; a history record for this plugin

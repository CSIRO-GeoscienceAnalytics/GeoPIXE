...
;
; name()				return name of device (e.g. "HS_PIXE_IXCCMAP_DEVICE").
...
pro HS_PIXE_IXCCMAP_DEVICE::cleanup
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'HS_PIXE_IXCCMAP_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
function HS_PIXE_IXCCMAP_DEVICE::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_IXCCMAP_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	;if H5F_IS_HDF5(strip_file_ext(file)+'.h5') eq 0 then begin
	;	warning,'HS_PIXE_IXCCMAP_DEVICE::get_header_info','Not a valid HDF5 file: '+strip_file_ext(file)+'.h5'
	;	self.header.error = 1
	;	return, self.header
	;endif
	
	; open HDF5 file
	;hdf5_id = h5f_open( strip_file_ext(file)+'.h5')
	;header_id = H5D_open(hdf5_id,'/Header')
	;header = H5D_read(header_id)
	
;	
;	
;	hdf5_id = HDF_SD_START(strip_file_ext(file)+'.h5', /read)
;
;;  get the indexes to the data
;	offset_id = HDF_SD_NAMETOINDEX(hdf5_id, 'OFFSET')
;	slope_id = HDF_SD_NAMETOINDEX(hdf5_id, 'SLOPE')
;	quad_id = HDF_SD_NAMETOINDEX(hdf5_id, 'QUAD')
;	units_id = HDF_SD_NAMETOINDEX(hdf5_id, 'UNITS')
;	i0_id = HDF_SD_NAMETOINDEX(hdf5_id, 'i0')
;
;;  select the SDs
;	sdsOFFSET = HDF_SD_SELECT(hdf5_id, offset_id)
;	sdsSLOPE = HDF_SD_SELECT(hdf5_id, slope_id)
;	sdsQUAD = HDF_SD_SELECT(hdf5_id, quad_id)
;	sdsUNITS = HDF_SD_SELECT(hdf5_id, units_id)
;	sdsI0 = HDF_SD_SELECT(hdf5_id, i0_id)
;
...
;n = min([geopixe_max_adcs,n_elements(header.OFFSET),32])
;	self.header.cal[0:n-1].on = 1
;self.header.cal[0:n-1].a = header.GAIN/1000
;self.header.cal[0:n-1].b = header.OFFSET/1000
...
;	title_id = HDF_SD_ATTRFIND(hdf5_id,'Image title')
;	ncols_id = HDF_SD_ATTRFIND(hdf5_id,'Number of columns')
;	nrows_id = HDF_SD_ATTRFIND(hdf5_id,'Number of rows')
;	time_id  = HDF_SD_ATTRFIND(hdf5_id, 'Preset Real Time (msec)')
;	start1_id  = HDF_SD_ATTRFIND(hdf5_id, 'Motor 1 Start')
;	stop1_id  = HDF_SD_ATTRFIND(hdf5_id, 'Motor 1 Stop')
;	start2_id  = HDF_SD_ATTRFIND(hdf5_id, 'Motor 2 Start')
;	stop2_id  = HDF_SD_ATTRFIND(hdf5_id, 'Motor 2 Stop')
;
;	HDF_SD_ATTRINFO, hdf5_id, title_id, DATA=title
;	HDF_SD_ATTRINFO, hdf5_id, ncols_id, DATA=ncols
;	HDF_SD_ATTRINFO, hdf5_id, nrows_id, DATA=nrows
;	HDF_SD_ATTRINFO, hdf5_id, time_id, DATA=ntime
;	HDF_SD_ATTRINFO, hdf5_id, start1_id, DATA=start1
;	HDF_SD_ATTRINFO, hdf5_id, stop1_id, DATA=stop1
;	HDF_SD_ATTRINFO, hdf5_id, start2_id, DATA=start2
;	HDF_SD_ATTRINFO, hdf5_id, stop2_id, DATA=stop2
;	HDF_SD_END, hdf5_id
;
;self.header.title = file
...
pro HS_PIXE_IXCCMAP_DEVICE::flux_scan, unit, evt_file, PV_list, IC_name, IC_val, IC_vunit, dwell=dwell, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_IXCCMAP_DEVICE::flux_scan',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
common c_hs_pixe_8, hdf5_id, sdsSPECTRA, hs_pixe_read_line
...
bad_io:
	warning,'HS_PIXE_IXCCMAP_DEVICE::flux_scan','HDF file I/O error.'
	error = 1
	return
end
...
function HS_PIXE_IXCCMAP_DEVICE::read_setup, unit, xrange, yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_IXCCMAP_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
	    ...
	    ;header_id = H5D_OPEN(hdf5_id,'/Header')
	    ;header = H5D_READ(header_id)
;	    HDF_SD_FILEINFO, hdf5_id, NumSDS, attributes
...
;	    units_id = HDF_SD_NAMETOINDEX(hdf5_id, 'UNITS')
;	    i0_id = HDF_SD_NAMETOINDEX(hdf5_id, 'i0')
	    ;spectra_id = H5D_OPEN(hdf5_id, '/Raw')
...
;	    sdsUNITS = HDF_SD_SELECT(hdf5_id, units_id)
;	    sdsI0 = HDF_SD_SELECT(hdf5_id, i0_id)
;	    sdsSPECTRA = HDF_SD_SELECT(hdf5_id, spectra_id)
;
;    ;	get the mca calibration info
;	    hdf_sd_getdata, sdsOFFSET, OFFSET
;	    hdf_sd_getdata, sdsSLOPE, SLOPE
...
;	    HDF_SD_ATTRINFO, hdf5_id, stop2_id, DATA=stop2
      ...
      SHMMAP, 'iXCCMap',[1024,264,264],/ULONG,OS_HANDLE='iXCCMap'
      datacube = SHMVAR('iXCCMap')
      cubedim = size(datacube,/dimensions)
      SHMUNMAP,'iXCCMap'
	  	hs_pixe_x_range = cubedim[1]
	  	hs_pixe_y_range = cubedim[2]
	  	hs_pixe_dets = 1
	  	hs_pixe_chans = cubedim[0]
	  	hs_pixe_x = 0
  		hs_pixe_y = 0
  		
  		hs_pixe_flux_scale = 1.0
;		flux2 = reform(I0, nCOLS,nROWS) * hs_pixe_flux_scale
...
;
      hs_pixe_dead = 0.0
;;		hs_pixe_dead = (time.real_time - time.live_time)/time.real_time
;;		q = where(finite(hs_pixe_dead) eq 0)
;;		if q[0] ne -1 then hs_pixe_dead[q]=0.0

  		ramp = indgen(hs_pixe_chans)
  		hs_pixe_e = uintarr( hs_pixe_chans, hs_pixe_dets)
  		hs_pixe_ste = hs_pixe_e
  		for i=0L,hs_pixe_dets-1 do begin
 		 	 hs_pixe_e[*,i] = ramp
 	 		 hs_pixe_ste[ramp,i] = i
  		endfor

 		hs_pixe_e = reform( hs_pixe_e, hs_pixe_chans*hs_pixe_dets)
 		hs_pixe_ste = reform( hs_pixe_ste, hs_pixe_chans*hs_pixe_dets)
 		hs_pixe_x1 = uintarr( hs_pixe_chans*hs_pixe_dets)
 		hs_pixe_y1 = uintarr( hs_pixe_chans*hs_pixe_dets)
...
;;	This will take up a lot of memory (approaching 1 Gb) for individual detectors.
 		...
 		;s = hs_pixe_x_range * hs_pixe_x_range * hs_pixe_chans * 4L /1000000L
 		;if s gt 300 then begin
 		;	warning,'HS_PIXE_IXCCMAP_DEVICE::read_setup','array in memory will be ',s,'Mbytes in size ...'
 		;endif

    hs_pixe_read_line = 0
    ;datacube = H5D_READ(spectra_id)
    ;hdf_sd_getdata, sdsSPECTRA, datacube, start=[0,0], count=[hs_pixe_x_range*hs_pixe_y_range,2048]
...
function HS_PIXE_IXCCMAP_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_IXCCMAP_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
	...
	nc = n_elements(channel_on)
	   ...
	   if (hs_pixe_x ge hs_pixe_x_range-1) and (hs_pixe_y ge hs_pixe_y_range-1) then begin
;		    print,'x,y=',hs_pixe_x,hs_pixe_y
      ;SHMUNMAP,'iXCCMap'
...
;raw_id = H5D_OPEN(file_id,'/Raw')
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	   read_by_line = 0
		if read_by_line and hs_pixe_read_line then begin
		  ;dataspace_id = H5D_GET_SPACE(spectra_id)
		  s = [0,0,i_buffer]
		  c = [hs_pixe_chans,hs_pixe_x_range,0]
		  ;H5S_SELECT_HYPERSLAB, dataspace_id, start, count, /RESET
		  ;mem_space_id = H5S_CREATE_SIMPLE(count)
		  SHMMAP, 'iXCCMapPart',[1024,264,264],/ULONG,OS_HANDLE='iXCCMap'
		  datacube = SHMVAR('iXCCMapPart')
		  datacube = datacube[s[0]:s[0]+c[0],s[1]:s[1]+c[1],s[2]:s[2]+c[2]]
		  SHMUNMAP,'iXCCMapPart'
		  ;datacube = H5D_READ(spectra_id, FILE_SPACE=dataspace_id, MEMORY_SPACE=mem_space_id)
;     hdf_sd_getdata, sdsSPECTRA, datacube, start=[i_buffer,0], count=[hs_pixe_x_range,2048]
			hs_pixe_read_line = 0
		endif else begin
		  SHMMAP, 'iXCCMapPart',[1024,264,264],/ULONG,OS_HANDLE='iXCCMap'
		  datacube = SHMVAR('iXCCMapPart')
...
;	       multiple = long( reform(datacube[hs_pixe_x,*], hs_pixe_chans*hs_pixe_dets))
	       multiple = long( reform(datacube[*,hs_pixe_x,0], hs_pixe_chans))
	   endif else begin
	       multiple = long( reform(datacube[*,hs_pixe_x,hs_pixe_y], hs_pixe_chans))
	   endelse
       ...
       hs_pixe_x = hs_pixe_x+1
       if hs_pixe_x ge hs_pixe_x_range then begin
         hs_pixe_x = 0
         hs_pixe_y = hs_pixe_y+1
         hs_pixe_read_line = 1
       endif
       i_buffer = i_buffer+1
...
function HS_PIXE_IXCCMAP_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_IXCCMAP_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'hs_pixe_ixccmap_evt': begin						; 41
		  SHMMAP, 'iXCCMap',[1024,264,264],/ULONG,OS_HANDLE='iXCCMap'
		  datacube = SHMVAR('iXCCMap')
		  SHMUNMAP,'iXCCMap'
		 ; file_id = H5F_OPEN(file)
		 ; spec_id = H5D_OPEN(file_id,'/Spectrum')
		 ; header_id = H5D_OPEN(file_id,'/Header')
		 ; p = H5D_READ(spec_id)
		 ; header = H5D_READ(header_id)
		  ;warning,'HS_PIXE_IXCCMAP_DEVICE::import_spec', p[111]
		  spec = define(/spectrum)
		  p = total(total(datacube,2),2)
		  spec.size = n_elements(p)
		  ;spec.size = header.spectrum
		  spec.data = ptr_new(p, /no_copy)
		  ;spec.cal.order = 1
		  ;spec.cal.poly[0] = header.offset/1000
		  ;spec.cal.poly[1] = header.gain/1000
		  ;spec.cal.units = 'keV'
		  spec.label = 'iXCCMap'
		  ;spec.comment = header.remark
		  p2 = ptrarr(1)
		  p2[0] = ptr_new(spec)
			;warning,'HS_PIXE_IXCCMAP_DEVICE::import_spec',['"hs_pixe_ixccmap_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p2
end
...
function HS_PIXE_IXCCMAP_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_41 = define(/import)			; HS_PIXE HDF image file
		opt_41.name =		'hs_pixe_ixccmap_evt'		; unique name of import
		opt_41.title =		'Extract from HS-PIXE Shared Memory Map'
		opt_41.in_ext =		''		; input file extension
		opt_41.request =	'Do not select anything'
	;	opt_41.raw =		1			; flags use of separate Raw data path '(*pstate).dpath'
		opt_41.spec_evt =	0			; uses a call to spec_evt to extract from EVT data
...
function HS_PIXE_IXCCMAP_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_IXCCMAP_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"HS_PIXE_IXCCMAP_DEVICE" --> HS_PIXE_IXCCMAP_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'HS_PIXE_IXCCMAP_DEVICE', $	; unique name for this device object
		title = 'HS-PIXE Shared Memory Map', $
		ext = '', $			; a fixed file extension for raw data
...
pro HS_PIXE_IXCCMAP_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('HS_PIXE_IXCCMAP_DEVICE')

COMPILE_OPT STRICTARR

maia = {HS_PIXE_IXCCMAP_DEVICE,  $

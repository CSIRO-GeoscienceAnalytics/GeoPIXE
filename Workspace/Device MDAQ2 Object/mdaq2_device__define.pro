;
; GeoPIXE Device Object for MDAQ2 listmode data
; 
; MDAQ2 is jamie Laird's FPGA-based pulse processor and data acquisition system. 
; Data is written by an NI front-end processor in little-endian byte order.
;
; Initialization:
...
;					Y LUT for MDAQ2 is a list of first Y value for each blog data file.
;
...
; method will be used in the "BASE_DEVICE" master-class. See the code in the MDAQ2_device
; as an example.
; 
...
; methods (see examples in MDAQ2_device). They are not essential and are NOT called
; from GeoPIXE:
; 
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'mdaq2_device::cleanup',['IDL run-time error caught.', '', $
...
pro mdaq2_device::render_options, parent
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::render_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
mdaq2mode_base = widget_base( parent, /column,  space=3, xpad=0, ypad=0, /base_align_center, $
		event_func='mdaq2_device_sort_option_event', uvalue=self, uname='obj-ref-here')
lab = widget_label( mdaq2mode_base, value='MDAQ2 Option Parameters')

;mdaq2tbase = widget_base( mdaq2mode_base, /row, /base_align_center, xpad=0, ypad=0, space=5)
;lab = widget_label( mdaq2tbase, value='Slowest motor axis (for YLUT):')
;mdaq2_slow = widget_combobox( mdaq2tbase, value=str_tidy(indgen(3)), uname='mdaq2-slow', /tracking, $
;					notify_realize='OnRealize_mdaq2_device_sort_option_slow', sensitive=1, $
...
mdaq2s0base = widget_base( mdaq2mode_base, /column, /base_align_center, xpad=1, ypad=1, space=3, xsize=axes_xsize, /frame)
lab = widget_label( mdaq2s0base, value='Select Motor Axis Indices for XYZ')
mdaq2sbase = widget_base( mdaq2s0base, /row, /base_align_center, xpad=0, ypad=0, space=5)
lab = widget_label( mdaq2sbase, value='X axis:')
mdaq2_xsource = widget_combobox( mdaq2sbase, value=str_tidy(indgen(3)), uname='mdaq2-x-source', /tracking, $
					notify_realize='OnRealize_mdaq2_device_sort_option_x_source', sensitive=1, $
					uvalue='Select the source motor axis to use as the GeoPIXE "X" axis.',xsize=source_xsize)
lab = widget_label( mdaq2sbase, value='  Y axis:')
mdaq2_ysource = widget_combobox( mdaq2sbase, value=str_tidy(indgen(3)), uname='mdaq2-y-source', /tracking, $
					notify_realize='OnRealize_mdaq2_device_sort_option_y_source', sensitive=1, $
...
lab = widget_label( mdaq2sbase, value='  Z axis:')
mdaq2_zsource = widget_combobox( mdaq2sbase, value=str_tidy(indgen(3)), uname='mdaq2-z-source', /tracking, $
					notify_realize='OnRealize_mdaq2_device_sort_option_z_source', sensitive=1, $
...
mdaq2cbase = widget_base( mdaq2mode_base, /row, /base_align_center, xpad=0, ypad=0, space=5)
lab = widget_label( mdaq2cbase, value='Clear Axis Border X:')
mdaq2_clearX = widget_combobox( mdaq2cbase, value=str_tidy(indgen(20)), uname='mdaq2-clear-x', /tracking, $
					notify_realize='OnRealize_mdaq2_device_sort_option_clear_x', sensitive=1, $
					uvalue='Clear both borders along X axis.',xsize=source_xsize)
lab = widget_label( mdaq2cbase, value='  Y:')
mdaq2_clearY = widget_combobox( mdaq2cbase, value=str_tidy(indgen(20)), uname='mdaq2-clear-y', /tracking, $
					notify_realize='OnRealize_mdaq2_device_sort_option_clear_y', sensitive=1, $
					uvalue='Clear both borders along Y axis.',xsize=source_xsize)
lab = widget_label( mdaq2cbase, value='  Z:')
mdaq2_clearZ = widget_combobox( mdaq2cbase, value=str_tidy(indgen(20)), uname='mdaq2-clear-z', /tracking, $
					notify_realize='OnRealize_mdaq2_device_sort_option_clear_z', sensitive=1, $
					uvalue='Clear both borders along Z axis.',xsize=source_xsize)
error = 0
;add_widget_vector, self.sort_id.slow, mdaq2_slow, error=err & error=error or err
add_widget_vector, self.sort_id.clear_X, mdaq2_clearX, error=err & error=error or err
add_widget_vector, self.sort_id.clear_Y, mdaq2_clearY, error=err & error=error or err
add_widget_vector, self.sort_id.clear_Z, mdaq2_clearZ, error=err & error=error or err
add_widget_vector, self.sort_id.source_x, mdaq2_xsource, error=err & error=error or err
add_widget_vector, self.sort_id.source_y, mdaq2_ysource, error=err & error=error or err
add_widget_vector, self.sort_id.source_z, mdaq2_zsource, error=err & error=error or err
if error then begin
	warning,'mdaq2_device::render_options','Error adding device object widget ID vectors.'
endif
return
end
...
pro OnRealize_mdaq2_device_sort_option_clear_x, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_mdaq2_device_sort_option_clear_x',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_clear_x: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_clear_x: Device Object ref not found.'
		return
	endif
...
pro OnRealize_mdaq2_device_sort_option_clear_y, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_mdaq2_device_sort_option_clear_y',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	ObjBase = find_id( wWidget, uname='obj-ref-here')
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_clear_y: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_clear_y: Device Object ref not found.'
		return
	endif
...
pro OnRealize_mdaq2_device_sort_option_clear_z, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_mdaq2_device_sort_option_clear_z',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	ObjBase = find_id( wWidget, uname='obj-ref-here')
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_clear_z: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_clear_z: Device Object ref not found.'
		return
	endif
...
;pro OnRealize_mdaq2_device_sort_option_slow, wWidget
;
;COMPILE_OPT STRICTARR
;common c_errors_1, catch_errors_on
;if n_elements(catch_errors_on) lt 1 then catch_errors_on=1
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'OnRealize_mdaq2_device_sort_option_slow',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		return
...
;	ObjBase = find_id( wWidget, uname='obj-ref-here')
;	if widget_info(ObjBase, /valid) eq 0L then begin
;		print,'OnRealize_mdaq2_device_sort_option_slow: Object base not found.'
;		return
;	endif
;	widget_control, ObjBase, get_uvalue=obj
;	if obj_valid(obj) eq 0L then begin
;		print,'OnRealize_mdaq2_device_sort_option_slow: Device Object ref not found.'
;		return
;	endif
;
;	options = obj->get_options()
...
pro OnRealize_mdaq2_device_sort_option_x_source, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_mdaq2_device_sort_option_x_source',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	ObjBase = find_id( wWidget, uname='obj-ref-here')
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_x_source: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_x_source: Device Object ref not found.'
		return
	endif
...
pro OnRealize_mdaq2_device_sort_option_y_source, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_mdaq2_device_sort_option_y_source',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	ObjBase = find_id( wWidget, uname='obj-ref-here')
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_y_source: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_y_source: Device Object ref not found.'
		return
	endif
...
pro OnRealize_mdaq2_device_sort_option_z_source, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_mdaq2_device_sort_option_z_source',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	ObjBase = find_id( wWidget, uname='obj-ref-here')
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_z_source: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_mdaq2_device_sort_option_z_source: Device Object ref not found.'
		return
	endif
...
function mdaq2_device_sort_option_event, event
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device_sort_option_event',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0L
...
;		print,'mdaq2_device_sort_option_event: event.handler not valid.'
		return, 0
	endif
	uname = widget_info( event.handler, /uname)
	if uname ne 'obj-ref-here' then begin
		print,'mdaq2_device_sort_option_event: Object base not found.'
		return, 0
	endif
	widget_control, event.handler, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'mdaq2_device_sort_option_event: Device Object ref not found.'
		return, 0
	endif
...
; "render_options" and the parameters read/written from/to DISK using the "read_options",
; "write_options" methods. Keep a local copy of device parameters and set them using
; "set_options". The keywords here are only used internally in MDAQ2 device.

pro mdaq2_device::set_options, p, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::set_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	if n_elements(mdaq2) eq 1 then begin
;		if tag_present('SLOW_AXIS',mdaq2) then self.sort_options.slow_axis = mdaq2.slow_axis
		if tag_present('CLEAR',mdaq2) then begin
			if tag_present('X',mdaq2.clear) then self.sort_options.clear.x = mdaq2.clear.x
			if tag_present('Y',mdaq2.clear) then self.sort_options.clear.y = mdaq2.clear.y
			if tag_present('Z',mdaq2.clear) then self.sort_options.clear.z = mdaq2.clear.z
		endif
		if tag_present('SOURCE',mdaq2) then begin
			if tag_present('X',mdaq2.source) then self.sort_options.source.x = mdaq2.source.x
			if tag_present('Y',mdaq2.source) then self.sort_options.source.y = mdaq2.source.y
			if tag_present('Z',mdaq2.source) then self.sort_options.source.z = mdaq2.source.z
		endif
	endif else begin
...
function mdaq2_device::get_options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
pro mdaq2_device::read_options, unit, options=options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::read_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
pro mdaq2_device::write_options, unit, options=p, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::write_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
function mdaq2_device::options_legend, p
...
function mdaq2_device::get_header_info, file, output=output, silent=silent, error=error
...
;			located in that path (e.g. MDAQ2 Y LUT). 
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;		retry = 0
;		ylut = get_mdaq2_ylut(file, output=output, error=err)	; stored now with output
;		if err or (n_elements(ylut) le 1) then retry=1	
;	endif else retry=1
;	if retry then begin
;		ylut = get_mdaq2_ylut( file, error=err)					; if not, try blog path
;	endif	
...
pro mdaq2_device::update_header_info, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::update_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	if ((*pmp).charge_tick gt 0.) then self.header.sensitivity = (*pmp).charge_tick * 1.e-6
		
;	if (*pmp).scan.xrange gt 0 then self.header.scan.on = 1
	self.header.scan.dwell = (*pmp).scan.dwell

	case self.sort_options.source.x of
		0: begin
			self.header.scan.x_pixels = (*pmp).scan.x_pixels
			self.header.scan.x_mm = (*pmp).scan.x_mm * 0.001
			self.header.scan.x = (*pmp).scan.x
			self.header.scan.x_on = (*pmp).scan.on
;			self.header.scan.x_name = (*pmp).scan.xname
			end
		1: begin
			self.header.scan.x_pixels = (*pmp).scan.y_pixels
			self.header.scan.x_mm = (*pmp).scan.y_mm * 0.001
			self.header.scan.x = (*pmp).scan.y
			self.header.scan.x_on = (*pmp).scan.on
...
;			end
	...
	case self.sort_options.source.y of
		0: begin
			self.header.scan.y_pixels = (*pmp).scan.x_pixels
			self.header.scan.y_mm = (*pmp).scan.x_mm * 0.001
			self.header.scan.y = (*pmp).scan.x
			self.header.scan.y_on = (*pmp).scan.on
;			self.header.scan.y_name = (*pmp).scan.xname
			end
		1: begin
			self.header.scan.y_pixels = (*pmp).scan.y_pixels
			self.header.scan.y_mm = (*pmp).scan.y_mm * 0.001
...
;			end
	...
	case self.sort_options.source.z of
		0: begin
			self.header.scan.z_pixels = (*pmp).scan.x_pixels
			self.header.scan.z_mm = (*pmp).scan.x_mm * 0.001
			self.header.scan.z = (*pmp).scan.x
			self.header.scan.z_on = (*pmp).scan.on
;			self.header.scan.z_name = (*pmp).scan.xname
			end
		1: begin
			self.header.scan.z_pixels = (*pmp).scan.y_pixels
			self.header.scan.z_mm = (*pmp).scan.y_mm * 0.001
			self.header.scan.z = (*pmp).scan.y
			self.header.scan.z_on = (*pmp).scan.on
...
;	self.header.detector[*] = -1
	...
	self.header.title = (*pmp).comment
	t = self.header.scan
	struct_assign, (*pmp).scan, t
	self.header.scan = t
	self.header.charge = (*pmp).charge
	self.header.detector = (*pmp).detector
	t = self.header.cal
	struct_assign, (*pmp).cal, t
	self.header.cal = t
	self.header.grain = (*pmp).name
...
function mdaq2_device::get_attribute_list, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_attribute_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
pro mdaq2_device::flux_scan, unit, evt_file, PV_list, IC_name, IC_val, IC_vunit, dwell=dwell, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::flux_scan',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
common c_mdaq2_2, mdaq2_swap,length,skip,x0,y0,z0,u0,v0,w0
common c_mdaq2_3, monitor_array, n_monitor, mdaq2_IC_name
common c_mdaq2_4, n_times, time_last
;common c_mdaq2_5, xanes_energies, do_xanes
common c_mdaq2_7, mdaq2_energy_pv
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
common c_mdaq2_17, mdaq2_x_range, mdaq2_y_range, mdaq2_z_range, mdaq2_3D
...
if n_elements(mdaq2_IC_name) lt 1 then mdaq2_IC_name=''
	...
	if self.header.sensitivity ne 0.0 then begin
		...
		print,'MDAQ2:flux_scan: found sensitivity = ', self.header.sensitivity
	endif
...
;
;		head = read_mdaq2_header( unit, error=err32)
;
...
;			mdaq2 = mdaq2_defaults(source='mdaq2_device::flux_scan', /any)		; read "mdaq2.conf" default PVs
;
...
;			check_plist_mdaq2, name				; what about extend 'val' too, or NOT check_plist yet?
;			if (n_elements(name) ge 1) and (mdaq2.epics.npic ge 1) then begin
;				q = where_tokens( *mdaq2.epics.pic, name, nq)
;				PV_list = name[q]
;				no_pv = 0
;			endif
;		endif
...
;			print,'mdaq2_device::flux_scan: deadtime_cal = ',deadtime_cal
;			
...
;				print,'MDAQ2:flux_scan: found sensitivity = ',head.IC_sensitivity
;			endif
;			
;		endif else begin
;			mdaq2_IC_name = ''				; in c_mdaq2_3
...
;;				nq = 0
;;				name = head.monitor.name
;;				check_plist_mdaq2, name
;;				if (n_elements(name) ge 1) and (mdaq2.epics.npic ge 1) then begin
;;					q = where_tokens( *mdaq2.epics.pic, name, nq)
;;				endIF
;	
;;				Ion chamber preamp sensitivity value
;;				Perhaps these should be in mdaq2 defaults read too?
;
...
;					nsls_flux_scale = 1.		; in c_nsls_?
;				endif else begin
;					mdaq2_IC_name = name[q[select.flux]]
;					IC_name = mdaq2_IC_name
;					if (nq2 ne 0) and (nq3 ne 0) then begin
...
;				endelse
;			endif
;			print,' mdaq2_device::flux_scan: mdaq2_IC_name = ',mdaq2_IC_name,', nsls_flux_scale = ', nsls_flux_scale
;		endelse
;	endif
;	error = 0
;	return
...
bad_io:
	warning,'mdaq2_device::flux_scan',['blog file I/O error','file = '+evt_file]
	error=1
	return
end
...
pro mdaq2_device::check_pv_list, plist

; Check the PV list pointed to by 'plist'. Add any MDAQ2 default PVs
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::check_pv_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
;function mdaq2_device::build_ylut, files, output=output, error=error, force=force
;
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'mdaq2_device::build_ylut',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		error = 1
...
;	
;build:	
;	ylut = build_mdaq2_ylut( files[0], slow_axis=self.sort_options.slow_axis, output=output, error=error)
;	
...
;pro mdaq2_device::delete_ylut, files, output=output, error=error
;
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'mdaq2_device::delete_ylut',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		error = 1
...
;endif
;
;	delete_mdaq2_ylut, files[0], output=output, error=error
;	return
;end

;-------------------------------------------------------------------

;function mdaq2_device::get_ylut, files, strip=strip, output=output, error=error
;
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'mdaq2_device::get_ylut',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		error = 1
...
;endif
;
;	ylut = get_mdaq2_ylut( files, strip=strip, output=output, error=error)
;	
;	nf = n_elements(files)
...
;function mdaq2_device::range_ylut, files, error=error
;
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'mdaq2_device::range_ylut',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		error = 1
...
;pro mdaq2_device::write_ylut, ylut, file, output=output, strip=strip, error=error
;
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'mdaq2_device::write_ylut',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		error = 1
...
;endif
;
;	write_mdaq2_ylut, ylut, file, output=output, strip=strip, error=error
;	return
;end
...
;function mdaq2_device::trim_evt_files, files, mask=pmask, pYlut=pYlut, $
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'mdaq2_device::trim_evt_files',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		return, ''
...
;    
;bad_files:
;	warning,'mdaq2_device::trim_evt_files','No MDAQ2 files contain the selected region(s).'
;	return, ''
;bad_index:
;	warning,'mdaq2_device::trim_evt_files',['Bad file YLUT index.','Data files exceed YLUT table.','', $
;			'Delete the old YLUT file, and it will be regenerated to include missing data files.']
;	return, ''
;bad_table:
;	warning,'mdaq2_device::trim_evt_files',['Bad YLUT lookup table for files: '+files[0]+' ...', $
...
function mdaq2_device::get_dead_weight, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_dead_weight',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
common c_mdaq2_19, mdaq2_weight

	error = 0
	if total(mdaq2_weight) lt 0.1 then error=1		; no raw counts found
...
function mdaq2_device::get_dead_weight_mode, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_dead_weight_mode',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
function mdaq2_device::get_dwell, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_dwell',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
function mdaq2_device::read_setup, unit, xrange,yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mdaq2_2, mdaq2_swap,length,skip,x0,y0,z0,u0,v0,w0
common c_mdaq2_3, monitor_array, n_monitor, mdaq2_IC_name
common c_mdaq2_11, mdaq2_hw_scaler, mdaq2_fixed_dwell
common c_mdaq2_12, mdaq2_flux_mode
common c_mdaq2_13, mdaq2_dwell
common c_mdaq2_19, mdaq2_weight
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
common c_mdaq2_17, mdaq2_x_range, mdaq2_y_range, mdaq2_z_range, mdaq2_3D
common c_mdaq2_18, fx3_last
common c_nsls_4, nsls_IC
common c_nsls_5, nsls_IC_value_index, nsls_flux_scale
common c_sandia_6, ibranch, hword,lword, RTCrec, DummyRec, adcs
common c_sandia_7, adc, tag, k_adc
...
if zrange gt 1 then mdaq2_3D=1
x_coord_units = ''
y_coord_units = ''
z_coord_units = ''
...
;			mdaq2 = mdaq2_defaults(source='mdaq2_device::read_setup', /any)		; read "MDAQ2.conf" default PVs
;			mdaq2_energy_pv = mdaq2.epics.energy
;			if mdaq2_energy_pv eq '' then mdaq2_energy_pv='ENERGY'
;			gprint,level=2,' mdaq2_device::read_setup: mdaq2_energy_pv = ',mdaq2_energy_pv
			
			self.spectrum_mode = 1
			if (n_elements(flux) ge 2) then self.spectrum_mode=0
			
;	Hardware counters "MDAQ2:scaler.FC0" and "MDAQ2.scaler.FC1" are already counts in a pixel. The scale needed
;	for these is just sensitivity val*unit. For Epics PV's in c/s we need to scale by dwell time (s).

			mdaq2_IC_name = flux_ic.pv
			if mdaq2_IC_name eq '' then mdaq2_IC_name='mdaq2:scaler.FC0'	; added 7/9/16 CGR

			mdaq2_hw_scaler = 0
			mdaq2_flux_mode = 0
			if strmid(mdaq2_IC_name,0,strlen('mdaq2:scaler')) eq 'mdaq2:scaler' then begin
				mdaq2_hw_scaler = 1
				mdaq2_flux_mode = 1
				if mdaq2_IC_name eq 'mdaq2:scaler.FC1' then mdaq2_flux_mode=2
			endif
			nsls_flux_scale = flux_ic.val * flux_ic.unit
			if (mdaq2_hw_scaler eq 0) and (self.spectrum_mode eq 0) and (xanes eq 0) then begin
				nsls_flux_scale = nsls_flux_scale * flux_ic.dwell*0.001		; scale by dwell only for maps
			endif
			mdaq2_fixed_dwell = flux_ic.dwell
			gprint,level=2,' mdaq2_device::read_setup: mdaq2_IC_name = ',mdaq2_IC_name,', nsls_flux_scale = ', nsls_flux_scale

;	Set-up an internal buffer to hold a dwell array, and a total elapsed time

			if self.spectrum_mode then begin
				mdaq2_dwell = flux[0]
				mdaq2_weight = dead_fraction							; dead-time weight array (by detector #)
			endif else begin
				mdaq2_dwell = flux[*,*,0]								; to replicate size and type as flux
				mdaq2_weight = dead_fraction							; dead-time weight array, same as dead_fraction
			endelse														; should dwell be zeroed here, or elsewhere?
			mdaq2_dwell[*] = 0.0										; zero only for 'first'
			mdaq2_weight[*] = 0.0										
			self.dwell_total = 0.0
			fx3_last = 0.
			
			mdaq2_swap = (big_endian() ne self->big_endian())

;	# buffers to read before Progress = 100,000/n_guide > 1. N_buffer MUST be divisable by 4 for Fortran code.
;	Take care increasing n_buffer, as the mdaq2_tt sum in spectrum mode will get large. Absolute limit is about 2M bytes.

			n_buffer = 50000L				; ~50K byte buffer (0.5 MBytes)
			n_guide = n_buffer				; progress every 10th buffer (5 Mb)
...
;;			head = read_mdaq2_32_header( unit, /no_scan, error=err32)
;			head = read_mdaq2_32_header( unit, error=err32)
;			if err32 eq 0 then begin
...
;			endif
			mdaq2_x_range = long(xrange)
			mdaq2_y_range = long(yrange)
			mdaq2_z_range = long(zrange)
		...
		tag = 0								; in c_sandia_7
		length = 0L							; in c_mdaq2_2
		skip = 0L							; in c_mdaq2_2
		i_buffer = 0L
...
function mdaq2_device::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, z1=z1, $
...
;   t			Time-over-threshold, for some DAQs (e.g. MDAQ2)
...
;
;	support_even_odd	=1 for MDAQ2 device (it supports selecting only even or odd Y rows)
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mdaq2_2, mdaq2_swap,length,skip,x0,y0,z0,u0,v0,w0
common c_mdaq2_3, monitor_array, n_monitor, mdaq2_IC_name
common c_mdaq2_6, mdaq2_y_min
common c_mdaq2_13, mdaq2_dwell
common c_mdaq2_19, mdaq2_weight
common c_mdaq2_15, mdaq2_fx, mdaq2_tags, mdaq2_pseudo
common c_mdaq2_16, mdaq2_0, mdaq2_1, mdaq2_2, mdaq2_3, mdaq2_4, mdaq2_5, mdaq2_e, mdaq2_t, mdaq2_ste, mdaq2_veto
common c_mdaq2_11, mdaq2_hw_scaler, mdaq2_fixed_dwell
common c_mdaq2_12, mdaq2_flux_mode
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
common c_mdaq2_17, mdaq2_x_range, mdaq2_y_range, mdaq2_z_range, mdaq2_3D
...
;		Find the number of active detectors selected 'nqc'
		...
		clock = ((self.clock gt 1.) and (self.clock lt 10000.)) ? self.clock : 80.

		on_ioerror, bad_io
		read_event_buffer, unit, self, n_actual, bytes=1
		if n_actual eq 0 then begin
			gprint,level=2,'mdaq2_device::read_buffer: bad n_actual'
			goto, bad_io
		endif
		i_buffer = i_buffer+1
		...
		if n_elements(x1) eq 0 then begin						; just assign these 
			...
			mdaq2_e = uintarr(n_events, /nozero)
			mdaq2_t = uintarr(n_events, /nozero)				; e,t per event
			mdaq2_0 = intarr(n_events, /nozero)					; x,y,z per event (unlike many devices, we will
			mdaq2_1 = intarr(n_events, /nozero)					; accept negative x,y,z,u,v,w)
			mdaq2_2 = intarr(n_events, /nozero)
			mdaq2_3 = intarr(n_events, /nozero)
			mdaq2_4 = intarr(n_events, /nozero)
			mdaq2_5 = intarr(n_events, /nozero)
			mdaq2_ste = uintarr(n_events, /nozero)				; 'station', i.e. detector #
			mdaq2_veto = uintarr(n_events, /nozero)				; flag a rejected event
			mdaq2_tags = uintarr(n_events, /nozero)				; can be used to retrieve debug event info
			mdaq2_pseudo = uintarr(n_events, /nozero)			; pseudo event (i.e. select, FC0, FC1, dwell, DT in records added as events)
			n_fx = 6
			mdaq2_fx = fltarr(n_fx,n_events, /nozero)			; selected flux (Epics or h/w), FC0, FC1, BT for each event
		endif else begin
			n_events = n_elements(mdaq2_e)
			n_fx = n_elements(mdaq2_fx[*,0])
		endelse
		n = 0L
		nm = 0L
		good = 0L
		...
		gprint, level=1, 'mdaq2_device::read_buffer: mdaq2_swap = ', mdaq2_swap
;		y0 = fix( y0 > yoffset)
...
;		NOTE: for encoder_y_mode=1 to work mdaq2_x_range must be actual scan X range.
;		Note that x0,y0,z0 keep current place of axis 0,1,2 (not really x,y,z).
		
		err = MDAQ2_events( event_array,n_actual, channel_on,nc, mdaq2_e,mdaq2_t,mdaq2_0,mdaq2_1,mdaq2_2, $
				mdaq2_3,mdaq2_4,mdaq2_5, mdaq2_ste,mdaq2_veto,mdaq2_tags,n_events,n, $
				mdaq2_fx,n_fx, mdaq2_flux_mode,clock, x0,y0,z0,u0,v0,w0, ibranch,mdaq2_swap, tag,length,skip, bad_xy, debug )

		nsls_debug = debug
		if err ne 0 then begin
			gprint,level=2,'mdaq2_device::read_buffer: error (',err,') return from MDAQ2_events1'
			if bad_xy then gprint,level=2,'		... and bad xy'
			goto, bad_io
		endif

		if n eq 0 then begin
			gprint,level=2,'mdaq2_device::read_buffer: Zero "n" return from MDAQ2_events6'
			return, 0
		endif
...
;			0: mdaq2_y_min = min(mdaq2_0[0:n-1])
;			1: mdaq2_y_min = min(mdaq2_1[0:n-1])
;			2: mdaq2_y_min = min(mdaq2_2[0:n-1])
;			3: mdaq2_y_min = min(mdaq2_3[0:n-1])
;			4: mdaq2_y_min = min(mdaq2_4[0:n-1])
;			5: mdaq2_y_min = min(mdaq2_5[0:n-1])
;		endcase
		mdaq2_y_min = 0
...
;		good	number of valid events, so far
		...
		mdaq2_pseudo[*] = 0
		if n_veto gt 0 then mdaq2_pseudo[qveto] = 1
...
;		Compress all, not just good events, so that pseudo events get corrected too ...
		...
		xrange2 = long( mdaq2_x_range / xcompress)
		yrange2 = long( mdaq2_y_range / ycompress)
		zrange2 = long( mdaq2_z_range / zcompress)
...
;			Combine all detectors, so divide DTcal by the number active 'nc'.
			...
			zmin = mdaq2_3D ? (self.sort_options.clear.z + (zoffset2 < 0)) : 0
...
;			Test border, and offset all, not just good events, so that pseudo events get offset ...
			...
			zbad = (mdaq2_3D eq 0) ? 0 : ((z1 lt zmin) or (z1 gt (zrange2-1-zmin)))
			...
			if nq1 gt 0 then begin
				...
				mdaq2_pseudo[q1] = 0				; also do not include flux, etc. for border
			endif
			q = where( veto eq 0, good)
...
;				0	Epics PV as a rate, so set the flux in a pixel to mdaq2_fx[0,*], if it is not zero
;						scaled by: nsls_flux_scale * float(xcompress)*float(ycompress)
;				1,2	H/W flux counter used and accumulated in mdaq2_fx[0,*] in Fortran, so accumulate mdaq2_fx[0,*] 
;						here in flux[*,*,0] scaled by: nsls_flux_scale
;				
;					Use FC0 in mdaq2_fx[1,*] to accumulate into flux[*,*,1]
;					Use FC1 in mdaq2_fx[2,*] to accumulate into flux[*,*,2]
;			
...
;
;			Dead_fraction:
;				Use fx[4,*] to set DT in dead_fraction, weighted by 1/nqc in imaging case (see above).
;				Later (in da_evt) norm this using the dwell (ms) array.

		 	if mdaq2_3D then begin
				mdaq2_accumulate_dtfx_3D, /image, x1,y1,z1,ste,veto,mdaq2_fx,n,n_fx, mdaq2_flux_mode, nsls_flux_scale, $
		 				n_elements(flux[*,0,0]),n_elements(flux[0,*,0]),n_elements(flux[0,0,*]), dta, dead_fraction, pseudo=mdaq2_pseudo, $
		 				flux=flux, weight=mdaq2_weight, dwell=mdaq2_dwell, xcompress=xcompress,ycompress=ycompress, error=err
		 	endif else begin
				mdaq2_accumulate_dtfx, /image, x1,y1,ste,veto,mdaq2_fx,n,n_fx, mdaq2_flux_mode, nsls_flux_scale, $
		 				n_elements(flux[*,0,0]),n_elements(flux[0,*,0]),n_elements(flux[0,0,*]), dta, dead_fraction, pseudo=mdaq2_pseudo, $
		 				flux=flux, weight=mdaq2_weight, dwell=mdaq2_dwell, xcompress=xcompress,ycompress=ycompress, error=err
		 	endelse
...
;			Remember to NOT skip veto events, some will be pseudo events with valid BT, FC0, FC1
			...
			if mdaq2_hw_scaler then begin							; flux is NOT suitable for Regions
				flux = flux + nsls_flux_scale * total(mdaq2_fx[0,0:n-1])
			endif 
			dt = total(mdaq2_fx[3,0:n-1])							; dwell time (ms)   ???

			mdaq2_accumulate_dtfx, /spectrum, x1,y1,ste,veto,mdaq2_fx,n,n_fx, mdaq2_flux_mode, nsls_flux_scale, $
 					n_elements(dead_fraction[*,0]),1,1, dta, dead_fraction, weight=mdaq2_weight, error=err
...
function mdaq2_device::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_import_spec',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'mdaq2_evt': begin							; 9
			warning,'mdaq2_device::import_spec',['"mdaq2_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p
end
...
function mdaq2_device::get_import_list, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;		yr:				200 $			; default Y range
	...
	opt_39 = define(/import)			; MDAQ2 384 list-mode
		opt_39.name =		'mdaq2_evt'	; unique name of import
		opt_39.title =		'Extract from MDQ - listmode files'
		opt_39.in_ext =		'.mdq'		; input file extension
		opt_39.request =	'MDQ data scan for all spectra [Enter FIRST file or range]'
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq2_device::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
	...
	self.header.scan.use_ylut = 0						; MDAQ2 device can use a Y lookup table?
	self.header.scan.pYlut = ptr_new(/allocate_heap)	; allocate heap for Y LUT
	
	self.clock	= 80.0									; 80 MHz clock default
...
;	self.options.scan.ysize = 140						; Y size of sort options box, when open (with slowest)
	self.options.scan.ysize = 110						; Y size of sort options box, when open (no slowest)
...
;	"mdaq2_DEVICE" --> mdaq2_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'mdaq2_DEVICE', $	; unique name for this device object
		title = 'CSIRO MDAQ2 - LabVIEW data acquisition', $
		ext = '.mdq', $			; fixed file extension for event data
		multi_files = 0, $		; multiple segment files per run
		multi_char = '', $		; separates run from segment number in file name
		big_endian = 0, $		; MDAQ2 data written with Linux byte order
		vax_float = 0, $		; not VAX floating point
		start_adc = 0, $		; start detector ADC #'s at 0 (channel # same as channel in files)
		use_bounds = 0, $		; not confine charge/flux within bounded area
		synchrotron = 0, $		; not synchrotron data
...
pro mdaq2_device__define

; Define MDAQ2 device object internal data structure.
; This adds some internal device parameters, local to MDAQ2 in 'sort_options',
; and device specific sort widgets options to be displayed in the 'Scan' tab
; of the Sort window and elsewhere in 'sort_id'.
; Only called using obj = obj_new('mdaq2_DEVICE')
...
mdaq2 = { mdaq2_DEVICE,  $
		...
		clock:	0.0, $										; FPGA clock (MHz)
			
		sort_options : {sort_options_devicespec_mdaq2, $	; Sort EVT window Sort options panel
				clear: {sort_options_clear_devicespec_mdaq2, $
...
;				slow_axis:	1 }, $							; slow axis for YLUT  (LATER?)
		...
		sort_id: {sort_id_mdaq2, $							; pointers to vector of sort widget IDs

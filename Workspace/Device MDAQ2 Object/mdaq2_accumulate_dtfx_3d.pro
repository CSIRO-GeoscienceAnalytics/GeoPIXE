pro mdaq2_accumulate_dtfx_3D, x,y,z,ste,veto,fx,n,n_fx, flux_mode, flux_scale, xrange,yrange,zrange, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'mdaq2_accumulate_dtfx_3D',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'mdaq2_accumulate_dtfx_3d'), cdecl=geolib_cdecl(), $
		image, x,y,z,ste,veto,pseudo,fx,long(n),long(n_fx), long(flux_mode), flux_scale, $
		long(xcompress),long(ycompress), long(xrange),long(yrange),long(zrange), float(dta), $
		flux, dead, weight, dwell, value=value )
if err then gprint,' mdaq2_accumulate_dtfx_3D: Error in Fortran - bad number of arguments'
return

bad:
	gprint,' mdaq2_accumulate_dtfx_3D: Error - bad type for argument: '+sb
	err = 1L
	return
end

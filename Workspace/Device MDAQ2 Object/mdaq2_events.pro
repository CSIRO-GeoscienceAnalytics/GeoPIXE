function mdaq2_events, ev,n_buffer, channel_on,nc, e,t,x,y,z,u,v,w,ste,veto,tags,n_events,n, $
			fx,n_fx, flux_mode,clock, x0,y0,z0,u0,v0,w0, ibranch,swap, tag,length,skip, bad,debug
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'mdaq2_events',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'mdaq2_events'), cdecl=geolib_cdecl(), $
			ev,long(n_buffer), channel_on,long(nc), e,t,x,y,z,u,v,w,ste,veto,tags,long(n_events),n, $
			fx,long(n_fx), long(flux_mode),float(clock), x0,y0,z0,u0,v0,w0, ibranch, long(swap), tag,length,skip, bad,debug, $ 
			value=value )

if err then begin
	print, 'mdaq2_events Fortran error'
	print,'		tag, length, ibranch, skip =',tag, length, ibranch, skip
endif
return, err

bad:
	gprint,'mdaq2_events: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

...
;
; name()				return name of device (e.g. "eds_hmsa_DEVICE").
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'eds_hmsa_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
pro eds_hmsa_device::set_options, p, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_device::set_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
function eds_hmsa_device::get_options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_device::get_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
pro eds_hmsa_device::read_options, unit, options=options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_device::read_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
pro eds_hmsa_device::write_options, unit, options=p, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_device::write_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
function eds_hmsa_device::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	mp = get_eds_hmsa_header( file, error=error)
	if error then begin
		self.header.error = 1
		return, self.header
	endif
...
pro eds_hmsa_DEVICE::update_header_info, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_DEVICE::update_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
;	self.header.scan.dwell = (*pmp).dwell
	...
	hmsa_dtype= (*pmp).dtype
	hmsa_doffset= (*pmp).doffset
	hmsa_nenergy= (*pmp).nenergy
...
function eds_hmsa_DEVICE::read_setup, unit, xrange, yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_hmsa_2, hmsa_nx, hmsa_ny
	...
	if err then begin
		warning,'eds_hmsa_DEVICE::read_setup','Failed to read XML file.'
		goto, bad_io
	endif
	...
	hmsa_dtype = head.data.ImageRaster_EDS_hypermap.DatumType
	hmsa_doffset = fix(head.data.ImageRaster_EDS_hypermap.DataOffset)

	hmsa_nenergy = fix(head.data.ImageRaster_EDS_hypermap.DatumDimensions.Dimension_Channel)
	hmsa_nx = fix(head.data.ImageRaster_EDS_hypermap.CollectionDimensions.Dimension_X)
	hmsa_ny = fix(head.data.ImageRaster_EDS_hypermap.CollectionDimensions.Dimension_Y)
...
;	Set up various vectors to use to construct X,Y,E events in 'read_buffer'
	...
	case hmsa_dtype of
		'uint16': begin
			...
			warning, 'eds_hmsa_DEVICE::read_setup', 'Unknown data type: '+hmsa_dtype
			goto, bad_io
			end
	endcase

	padding = bytarr(hmsa_doffset)
	readu, unit, padding
	return, 0	
...
function eds_hmsa_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
function eds_hmsa_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
case name of
	'eds_hmsa_evt': begin							; 9
		warning,'eds_hmsa_DEVICE::import_spec',['"eds_hmsa_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
		end
	endcase
	return, p2
end
...
function eds_hmsa_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_39 = define(/import)			; XFM new HDF5 file read as a list-mode
		opt_39.name =		'eds_hmsa_evt'	; unique name of import
		opt_39.title =		'Extract E,X,Y from EDS map HMSA file as list-mode'
		opt_39.in_ext =		'.hmsa'		; input file extension
		opt_39.request =	'Select EDS map HMSA file to scan for all spectra, X,Y'
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'eds_hmsa_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"eds_hmsa_DEVICE" --> eds_hmsa_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'eds_hmsa_DEVICE', $	; unique name for this device object
		title = 'EDS map data HMSA file', $
		ext = '.hmsa', $			; a fixed file extension for raw data
...
pro eds_hmsa_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('eds_hmsa_DEVICE')

COMPILE_OPT STRICTARR

maia = { eds_hmsa_DEVICE,  $
...
;		write and read of device options, which is defined in the 'write_options' method.
		...
		sort_options : {sort_options_devicespec_eds_hmsa, $	; Sort EVT window Sort options panel
				version:	0.0 } $		; HMSA/XML version

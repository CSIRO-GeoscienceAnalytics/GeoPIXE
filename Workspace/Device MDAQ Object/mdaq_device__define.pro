;
; GeoPIXE Device Object for MicroDAQ system data
;
; MicroDAQ is the LabVIEW data acquisition and scanning
; system developed by Jamie Laird for general nuclear microprobe imaging.
; Data is written by a Windows PC processor in little-endian byte order.
; 
; Initialization:
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'mdaq_device::cleanup',['IDL run-time error caught.', '', $
...
function mdaq_device::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq_device::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
function mdaq_device::read_setup, unit, xrange,yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq_device::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
function mdaq_device::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq_device::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
	...
	err = mdaq_events( event_array,n_buffer,channel_on,nc, e,x,y,n,ste, bad_xy )

	if err ne 0 then begin
		print,'read_buffer: error (',err,') return from mdaq_events'
		goto, bad_io
	endif
...
function mdaq_device::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq_device::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
;		'mdaq_mu_ascii': begin					; 6
;			get_mu_spec, p, file
;			end
		'mdaq_evt': begin						; 9
			warning,'mdaq_device::import_spec',['"mdaq_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p
...
;			if (mp.charge gt 1.0e-6) and ( (opt[index]).use_ic eq 0) then (*p[i]).charge = mp.charge
			(*p[j]).charge = mp.charge

			detector = ['PIXE','']
			q = where( mp.detector eq detector, nq)
			if nq gt 0 then (*p[j]).detector = q[0] 
			done[adc] = 1
		endfor
	endif
...
function mdaq_device::get_import_list, error=error
...
;	opt_1 = define(/import)					; MicroDAQ/Linux IMG
;		opt_1.name =		'mdaq_img'		; unique name of import
;		opt_1.title =		'MicroDAQ/Linux IMG file'
;		opt_1.in_ext =		'.img'			; file extension
;		opt_1.request =		'Select IMG to convert and load'
;	
;	opt_6 = define(/import)					; MU TXT
;		opt_6.name =		'mdaq_mu_ascii' ; unique name of import
;		opt_6.title =		'MARC ascii spectrum'
;		opt_6.in_ext =		'.txt'			; file extension
;		opt_6.request =		'Select MU Spectrum to Load'
	
	opt_9 = define(/import)					; MicroDAQ/Linux EVT
		opt_9.name =		'mdaq_evt'		; unique name of import
		opt_9.title =		'Extract from MicroDAQ/LabVIEW EVT'
		...
		opt_9.IC_mode = 	0				; direct charge measurement
	
;	opt_17 = define(/import)			; Rontec ASCII spec
;		opt_17.name =		'mdaq_rontec_txt'		; unique name of import
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'mdaq_device::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"mdaq_device" --> mdaq_device_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'MDAQ_DEVICE', $	; unique name for this device object
		title = 'CSIRO MicroDAQ - LabVIEW data acquisition', $
...
pro mdaq_device__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('mdaq_device')

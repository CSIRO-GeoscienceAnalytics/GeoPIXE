;
; GeoPIXE Device Object for NSLS HDF data
; 
; Initialization:
...
;
; name()				return name of device (e.g. "NSLS_HDF_DEVICE").
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'NSLS_HDF_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
function NSLS_HDF_DEVICE::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'NSLS_HDF_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
pro NSLS_HDF_DEVICE::flux_scan, unit, evt_file, PV_list, IC_name, IC_val, IC_vunit, dwell=dwell, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'NSLS_HDF_DEVICE::flux_scan',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
bad_io:
	warning,'NSLS_HDF_DEVICE::flux_scan','HDF file I/O error.'
	error = 1
	return
end
...
function NSLS_HDF_DEVICE::read_setup, unit, xrange,yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'NSLS_HDF_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
;	This will take up a lot of memory (approaching 1 Gb) for individual detectors.
		...
		if s gt 300 then begin
			warning,'NSLS_HDF_DEVICE::read_setup','array in memory will be ',s,'Mbytes in size ...'
		endif
        nsls_read_line = 0
...
function NSLS_HDF_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'NSLS_HDF_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
function NSLS_HDF_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'NSLS_HDF_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'nsls_hdf_evt': begin						; 41
			warning,'NSLS_HDF_DEVICE::import_spec',['"nsls_hdf_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p
end
...
function NSLS_HDF_DEVICE::get_import_list, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'NSLS_HDF_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"NSLS_HDF_DEVICE" --> NSLS_HDF_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'NSLS_HDF_DEVICE', $	; unique name for this device object
		title = 'NSLS HDF - SXRF image data file', $
...
pro NSLS_HDF_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('NSLS_HDF_DEVICE')

COMPILE_OPT STRICTARR

maia = {NSLS_HDF_DEVICE,  $

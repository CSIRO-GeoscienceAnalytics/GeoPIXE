;
; GeoPIXE Device Object for Ruder Boskovic Institute, Zagreb LST format data
;
; Initialization:
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'ZAGREB_LST_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
function ZAGREB_LST_DEVICE::read_setup, unit, xrange,yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ZAGREB_LST_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
		...
		event_array = uintarr( 3, blocks*256, /nozero)
...
function ZAGREB_LST_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ZAGREB_LST_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
	...
	nc = n_elements(channel_on)
       ...
       e = uint( event_array[0,0:n_actual-1] AND e_mask)
       ste = uint( ishft(event_array[0,0:n_actual-1], ste_offset)-1 > 0 )
       x1 = uint( event_array[1,0:n_actual-1] AND x_mask)
       y1 = uint( event_array[2,0:n_actual-1] AND y_mask)
       n = n_actual
...
function ZAGREB_LST_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ZAGREB_LST_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'zagreb_lst_evt': begin						; 33
			warning,'ZAGREB_LST_DEVICE::import_spec',['"zagreb_lst_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p
end
...
function ZAGREB_LST_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_33 = define(/import)			; Zagreb LST
		opt_33.name =		'zagreb_lst_evt'		; unique name of import
		opt_33.title =		'Extract from Zagreb LST'
		opt_33.in_ext =		'.lst'		; input file extension
		opt_33.request =	'Select Zagreb LST file to scan for all spectra'
		opt_33.raw =		1			; flags use of separate Raw data path '(*pstate).dpath'
		opt_33.spec_evt =	1			; uses a call to spec_evt to extract from EVT data
...
function ZAGREB_LST_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'ZAGREB_LST_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"ZAGREB_LST_DEVICE" --> ZAGREB_LST_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'ZAGREB_LST_DEVICE', $	; unique name for this device object
		title = 'Ruder Boskovic Institute - LST data', $
...
pro ZAGREB_LST_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('ZAGREB_LST_DEVICE')

COMPILE_OPT STRICTARR

maia = {ZAGREB_LST_DEVICE,  $

;
;  even_rows Image plugin routine
;  -----------------------------
;
...
pro even_rows_image_plugin, p, ii, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = '* Even Rows Image Plugin'		; return the menu title for this plugin
	return
endif
...
for j=0,sy-2,2 do begin
	if j eq sy-2 then begin
		(*pimg)[*,j+1,*] = (*pimg)[*,j,*]
	endif else begin
		(*pimg)[*,j+1,*] = 0.5*((*pimg)[*,j,*] + (*pimg)[*,j+2,*])
	endelse
endfor

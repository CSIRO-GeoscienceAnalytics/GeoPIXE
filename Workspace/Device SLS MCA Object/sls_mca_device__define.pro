;
; GeoPIXE Device Object for SLS MCA format image data (basic only, needs work ...)
;
; Initialization:
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'SLS_MCA_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
function SLS_MCA_DEVICE::read_setup, unit, xrange,yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'SLS_MCA_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
;	Should also read ???_infos_??.dat file to read I0 and dead-time tables for pixels ...
;	Use these set values in the 'flux' and 'dead_fraction' arrays.

		on_ioerror, bad_io
		s = ''
		set_separators, ' 	'
		readf, unit,s					; skip date title and blank line
		readf, unit,s

		readf, unit,s					; read first pixel spectrum
		chop_string,s,sub,n_sub
		print, 'n_sub=',n_sub
		if n_sub lt 256 then goto, bad_io
		...
		nsls_dets = 1
		nsls_chans = n_sub-1
		nsls_data = replicate( {spectrum:lonarr(nsls_chans), trail:''}, nsls_dets)
...
function SLS_MCA_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'SLS_MCA_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_nsls_9, nsls_debug
	...
	nc = n_elements(channel_on)
	   ...
	   readf, unit, nsls_data

       multiple = long( reform(nsls_data[*].spectrum, nsls_chans*nsls_dets))
       e = nsls_e
       ste = nsls_ste
       x1 = nsls_x1
       y1 = nsls_y1
...
function SLS_MCA_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'SLS_MCA_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'sls_mca_evt': begin					; 42
			warning,'SLS_MCA_DEVICE::import_spec',['"sls_mca_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p
end
...
function SLS_MCA_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_42 = define(/import)			; SLS MCA
		opt_42.name =		'sls_mca_evt'		; unique name of import
		opt_42.title =		'Extract from SLS MCA image file'
		opt_42.in_ext =		'.mca'		; input file extension
		opt_42.request =	'Select SLS MCA file to scan for all spectra'
		opt_42.raw =		1			; flags use of separate Raw data path '(*pstate).dpath'
		opt_42.spec_evt =	1			; uses a call to spec_evt to extract from EVT data
		opt_42.xr =			200			; default X range
		opt_42.yr =			200			; default Y range
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'SLS_MCA_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"SLS_MCA_DEVICE" --> SLS_MCA_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'SLS_MCA_DEVICE', $	; unique name for this device object
		title = 'SLS MCA - SXRF image data file', $
		ext = '.mca', $			; file extension for raw data
...
pro SLS_MCA_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('SLS_MCA_DEVICE')

;
; GeoPIXE Device Object for Fastcom MPA4 system data as used at Sandia
;
; MPA4 is a 4-16 channel data acquisition and scanning
...
; use of a Y lookup table, which can be flagged this way (not used in Fastcom_MPA4 as yet):
;
;	self.pileup.use = 1
;	self.throttle.use = 1
;	self.linear.use = 1
...
; method will be used in the "BASE_DEVICE" master-class. See the code in the fastcom_MPA4_device
; as an example.
; 
...
; methods (see examples in fastcom_MPA4_device). They are not essential and are NOT called
; from GeoPIXE:
; 
...
pro FASTCOM_MPA4_DEVICE::cleanup
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'FASTCOM_MPA4_DEVICE::cleanup',['IDL run-time error caught.', '', $
	...
	if ptr_valid( self.sort_id.charge_adc_id) then ptr_free, self.sort_id.charge_adc_id
...
pro FASTCOM_MPA4_DEVICE::render_options, parent
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::render_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
mpa4mode_base = widget_base( parent, /column,  space=3, xpad=0, ypad=0, /base_align_center, $
		event_func='FASTCOM_MPA4_DEVICE_sort_option_event', uvalue=self, uname='obj-ref-here')
lab = widget_label( mpa4mode_base, value='MPA4 Option Parameters')

; A droplist (combobox) of charge ADC. The uvalue contains a help string to be displayed to explain this widget.

mpa4_adc_base = widget_base( mpa4mode_base, /row, /base_align_center, xpad=0, ypad=0, space=5)

lab = widget_label( mpa4_adc_base, value='Charge pulse ADC:')

charge_adc_id = widget_combobox( mpa4_adc_base, value='  '+str_tidy(indgen(8)+1)+' ', uname='charge-adc-mode', /tracking, $
					notify_realize='OnRealize_fastcom_MPA4_device_sort_option_charge_adc',xsize=encoder_xsize,  $
					uvalue='Select the ADC channel to use for "charge pulses".')

; A droplist (combobox) of charge map smoothing. The uvalue contains a help string to be displayed to explain this widget.

mpa4_charge_base = widget_base( mpa4mode_base, /row, /base_align_center, xpad=0, ypad=0, space=5)

lab = widget_label( mpa4_charge_base, value='Smooth Charge map:')

charge_smooth_id = widget_combobox( mpa4_charge_base, value=['none','  '+str_tidy(indgen(10)+2)+' '], uname='charge-smooth-mode', /tracking, $
					notify_realize='OnRealize_fastcom_MPA4_device_sort_option_charge_smooth',xsize=encoder_xsize,  $
					uvalue='Select size of Boxcar smooth to deal with low statistics in charge map.')

; A droplist (combobox) of dead-fraction map smoothing. The uvalue contains a help string to be displayed to explain this widget.

mpa4_dead_base = widget_base( mpa4mode_base, /row, /base_align_center, xpad=0, ypad=0, space=5)

lab = widget_label( mpa4_dead_base, value='Smooth Dead-fraction map:')

dead_smooth_id = widget_combobox( mpa4_dead_base, value=['none','  '+str_tidy(indgen(10)+2)+' '], uname='dead-smooth-mode', /tracking, $
					notify_realize='OnRealize_fastcom_MPA4_device_sort_option_dead_smooth',xsize=encoder_xsize,  $
					uvalue='Select size of Boxcar smooth to deal with low statistics in dead-fraction map.')

error = 0
add_widget_vector, self.sort_id.charge_adc_id, charge_adc_id, error=err & error=error or err
add_widget_vector, self.sort_id.charge_smooth_id, charge_smooth_id, error=err & error=error or err
add_widget_vector, self.sort_id.dead_smooth_id, dead_smooth_id, error=err & error=error or err

if error then begin
	warning,'fastcom_MPA4_device::render_options','Error adding device object widget ID vectors.'
endif
return
end
...
pro OnRealize_fastcom_MPA4_device_sort_option_charge_adc, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_fastcom_MPA4_device_sort_option_charge_adc',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_fastcom_MPA4_device_sort_option_charge_adc: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_fastcom_MPA4_device_sort_option_charge_adc: Device Object ref not found.'
		return
	endif

	options = obj->get_options()
	
	widget_control, wWidget, set_combobox_select=options.charge_adc_mode
	return
end

;------------------------------------------------------------------------------

pro OnRealize_fastcom_MPA4_device_sort_option_charge_smooth, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_fastcom_MPA4_device_sort_option_charge_smooth',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	ObjBase = find_id( wWidget, uname='obj-ref-here')
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_fastcom_MPA4_device_sort_option_charge_smooth: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_fastcom_MPA4_device_sort_option_charge_smooth: Device Object ref not found.'
		return
	endif

	options = obj->get_options()
	
	widget_control, wWidget, set_combobox_select=options.charge_smooth_mode
	return
end

;------------------------------------------------------------------------------

pro OnRealize_fastcom_MPA4_device_sort_option_dead_smooth, wWidget
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'OnRealize_fastcom_MPA4_device_sort_option_dead_smooth',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	ObjBase = find_id( wWidget, uname='obj-ref-here')
	if widget_info(ObjBase, /valid) eq 0L then begin
		print,'OnRealize_fastcom_MPA4_device_sort_option_dead_smooth: Object base not found.'
		return
	endif
	widget_control, ObjBase, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'OnRealize_fastcom_MPA4_device_sort_option_dead_smooth: Device Object ref not found.'
		return
	endif

	options = obj->get_options()
	
	widget_control, wWidget, set_combobox_select=options.dead_smooth_mode
	return
end

;------------------------------------------------------------------------------------------

function fastcom_MPA4_device_sort_option_event, event
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'fastcom_MPA4_device_sort_option_event',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0L
...
;		print,'fastcom_MPA4_device_sort_option_event: event.handler not valid.'
		return, 0
	endif
	uname = widget_info( event.handler, /uname)
	if uname ne 'obj-ref-here' then begin
		print,'fastcom_MPA4_device_sort_option_event: Object base not found.'
		return, 0
	endif
	widget_control, event.handler, get_uvalue=obj
	if obj_valid(obj) eq 0L then begin
		print,'fastcom_MPA4_device_sort_option_event: Device Object ref not found.'
		return, 0
	endif
...
case tag_names( event,/structure) of
	'WIDGET_TRACKING': begin
		...
		case uname of
			'charge-adc-mode': begin
				obj->set_options, charge_adc_mode = event.index
				end
			'charge-smooth-mode': begin
				obj->set_options, charge_smooth_mode = event.index
				end
			'dead-smooth-mode': begin
				obj->set_options, dead_smooth_mode = event.index
				end
		endcase		
		end
endcase
...
pro fastcom_MPA4_device::set_options, p, charge_adc_mode=charge_adc_mode, charge_smooth_mode=charge_smooth_mode, $
											dead_smooth_mode=dead_smooth_mode
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'fastcom_MPA4_device::set_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	if n_elements(maia) eq 1 then begin
		if tag_present('CHARGE_ADC_MODE',maia) then self.sort_options.charge_adc_mode = maia.charge_adc_mode
		if tag_present('CHARGE_SMOOTH_MODE',maia) then self.sort_options.charge_smooth_mode = maia.charge_smooth_mode
		if tag_present('DEAD_SMOOTH_MODE',maia) then self.sort_options.dead_smooth_mode = maia.dead_smooth_mode
	endif else begin
		if n_elements(charge_adc_mode) eq 1 then self.sort_options.charge_adc_mode = charge_adc_mode
		if n_elements(charge_smooth_mode) eq 1 then self.sort_options.charge_smooth_mode = charge_smooth_mode
		if n_elements(dead_smooth_mode) eq 1 then self.sort_options.dead_smooth_mode = dead_smooth_mode
	endelse
	...
	widget_control_vector, self.sort_id.charge_adc_id, set_combobox_select = self.sort_options.charge_adc_mode
	widget_control_vector, self.sort_id.charge_smooth_id, set_combobox_select = self.sort_options.charge_smooth_mode
	widget_control_vector, self.sort_id.dead_smooth_id, set_combobox_select = self.sort_options.dead_smooth_mode
	return
end
...
function fastcom_MPA4_device::get_options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'fastcom_MPA4_device::get_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
pro fastcom_MPA4_device::read_options, unit, options=options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'fastcom_MPA4_device::read_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
; Get current defaults in this device
	...
	charge_adc_mode = options.charge_adc_mode
	charge_smooth_mode = options.charge_smooth_mode
	dead_smooth_mode = options.dead_smooth_mode
	version = 0L
...
; Read options parameters from the file
	...
	readu, unit, charge_adc_mode
	charge_adc_mode = clip(charge_adc_mode,0,7)
	
	if self.use_version then begin
		if version le -2 then begin
			readu, unit, charge_smooth_mode, dead_smooth_mode
			charge_smooth_mode = clip(charge_smooth_mode,0,10)
			dead_smooth_mode = clip(dead_smooth_mode,0,10)
		endif
	endif
	
; Write back these options parameters to the device ...

	options.charge_adc_mode = charge_adc_mode
	options.charge_smooth_mode = charge_smooth_mode
	options.dead_smooth_mode = dead_smooth_mode
...
pro fastcom_MPA4_device::write_options, unit, options=p, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'fastcom_MPA4_device::write_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
	...
	writeu, unit, options.charge_adc_mode
	writeu, unit, options.charge_smooth_mode, options.dead_smooth_mode
	error = 0
	return
...
function fastcom_MPA4_device::options_legend, p
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'fastcom_MPA4_device::options_legend',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, ''
	...
	on_off = ['Off','On']
	smooth = [0,2+indgen(10)]

	list = ['Fastcom MPA4:' ] 
	list = [list, '   Charge ADC channel: ' + str_tidy(options.charge_adc_mode + 1) ]
	list = [list, '   Charge smooth size: ' + str_tidy(smooth[options.charge_smooth_mode]) ]
	list = [list, '   Dead-fraction smooth size: ' + str_tidy(smooth[options.dead_smooth_mode]) ]
...
function FASTCOM_MPA4_DEVICE::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_sandia_4, sync, TimerEvent, RTCmask, Dummymask, maxADCs, mpa_x_adc, mpa_y_adc
	...
	mp = get_mpa4_header( file, error=error)
	...
	self.header.scan.x_pixels = mp.range[2]				; for ADC 3
	self.header.scan.y_pixels = mp.range[3]				; for ADC 4
	
	self.header.error = 0
	if ptr_valid( self.old_mp) then ptr_free, self.old_mp
	self.old_mp = ptr_new( mp, /no_copy)
	return, self.header
end

;-------------------------------------------------------------------

function FASTCOM_MPA4_DEVICE::read_setup, unit, xrange,yrange, first=first, $
...
;
;	/first		for first file in multi-file data-set (not used here)
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mpa4_1, ascii_event_array, use_ascii
...
;		"read_buffer".
		...
		on_ioerror, bad_io
		head = read_mpa4_header( unit, error=error)
		...
		readu, unit, tmp					; skip over header past [DATA]
...
;		n_buffer = 8L
		...
		if head.format eq 'asc' then begin
			use_ascii = 1
			ascii_event_array = strarr(n_buffer)
			event_array = ulon64arr( n_buffer)
		endif else begin
			use_ascii = 0
			event_array = ulon64arr( n_buffer)
		endelse
...
function FASTCOM_MPA4_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
       ecompress=ecompress, total_bad_xy=bad_xy, total_processed=processed, $
       station_e=ste, title=title, multiple=multiple, veto=veto, $
...
;   beam_energy	beam energy, if it's available (MeV=ionbeam, keV=synchrotron)
;   good		number of good events (same as 'n'), or zero
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_maia_16, maia_0, maia_1, maia_2, maia_e, maia_t, maia_ste, maia_veto
	...
	if nc eq 0 then goto, bad_io
	
	read_event_buffer, unit, self, n_actual, bytes=8, ascii_event_array=ascii_event_array, use_ascii=use_ascii
	if n_actual eq 0 then goto, bad_io
	i_buffer = i_buffer+1

;	maia_fx used for pseudo events:
;		0,1: busy, weight - for dead_fraction
;		2: charge - for charge counts

	if n_elements(x1) eq 0 then begin						; just assign these 
		n_events = n_buffer									; vectors first time
		maia_e = uintarr(n_events)
		maia_0 = lonarr(n_events)
		maia_1 = lonarr(n_events)							; x,y per event
		maia_ste = uintarr(n_events)						; ADC # (starts at zero)
		maia_veto = uintarr(n_events)						; flag a rejected event
		maia_pseudo = uintarr(n_events)						; pseudo event (i.e. Busy, Count, Charge in records added as events)
		n_fx = 3
		maia_fx = lonarr(n_fx,n_events)						; busy, weight, charge for each event
	endif else begin
	...
	n = 0L
	good = 0L
	charge_adc = self.sort_options.charge_adc_mode
...
;	good	number of valid events, so far (veto=0)
	...
	err = mpa4_events2( event_array, n_buffer,n_actual, maia_e,maia_0,maia_1,maia_ste, $
			maia_veto,maia_fx,n_fx, n_events,n, x0,y0, charge_adc, channel_on,nc, j_mpa,ibranch, bad_xy)

	if err ne 0 then begin
		print,'FASTCOM_MPA4_DEVICE::read_buffer: error (',err,') return from mpa4_events2'
		goto, bad_io
	endif

	if n eq 0 then begin
		print,'FASTCOM_MPA4_DEVICE::read_buffer: Zero "n" return from mpa4_events2'
		return, 0
	endif
	...
	x1 = uint( maia_0[0:n-1] + 1000)
	y1 = uint( maia_1[0:n-1] + 1000)

	good = n
	xoffset2 = xoffset
	yoffset2 = yoffset

	q = where( veto eq 0, good, complement=qveto, ncomplement=n_veto)	; good events
	maia_pseudo[*] = 0
	...
	if nt1 gt 0 then begin
		...
		qt = where( maia_pseudo[qt1] eq 1,nt4)				; psuedo events w/ positive x1,y1
		qt4 = qt1[qt]
	endif

	e = e AND '1FFF'x										; mask off top 3 bits
	...
	mpa4_accumulate_dtfx, x1,y1,ste,veto,n, maia_fx,n_fx, channel_on,nc, $
			xrange=n_elements(maia_weight[*,0]), yrange=n_elements(maia_weight[0,*]), dead=dead_fraction, $
			flux=flux, spectrum=self.spectrum_mode, pseudo=maia_pseudo, weight=maia_weight, error=err

	if self.spectrum_mode then begin

;		Accumulate total time in spec mode

		dt = total(maia_fx[1,0:n-1])						; dwell time (ms)
...
function FASTCOM_MPA4_DEVICE::get_dead_weight, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::get_dead_weight',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
common c_maia_19, maia_weight
	...
	weight = self->smooth_weight(maia_weight)
	return, weight
end

;-------------------------------------------------------------------

function FASTCOM_MPA4_DEVICE::get_dead_weight_mode, error=error

; Return the internal dead-time weight mode
;		0	weight is incoming count estimate
;		1	weight is outgoing raw count weight
;
; In this case use mode=0 as this Timer event is periodic and not subject to saturation.
; In effect, it just 'samples' busy or dead-fraction.
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::get_dead_weight_mode',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
function FASTCOM_MPA4_DEVICE::get_dwell, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::get_dwell',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		error = 1
...
common c_maia_19, maia_weight
	...
	error = 0
	dwell = self->smooth_weight(maia_weight)
	return, dwell
end
...
function FASTCOM_MPA4_DEVICE::smooth_flux, flux

COMPILE_OPT STRICTARR

	smooth = [0,2+indgen(10)]

	return, smooth( flux, smooth[self.sort_options.charge_smooth_mode], /edge_truncate )
end
...
function FASTCOM_MPA4_DEVICE::smooth_weight, weight

COMPILE_OPT STRICTARR

	smooth = [0,2+indgen(10)]

	return, smooth( weight, smooth[self.sort_options.dead_smooth_mode], /edge_truncate )
end

;-------------------------------------------------------------------

function FASTCOM_MPA4_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'fastcom_mpa4_evt': begin						; 9
			warning,'FASTCOM_MPA4_DEVICE::import_spec',['"fastcom_mpa4_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p
end
...
function FASTCOM_MPA4_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_22 = define(/import)			; Sandia MPA4 LST
		opt_22.name =		'fastcom_mpa4_evt'		; unique name of import
		opt_22.title =		'Extract from Madrid Fastcom MPA4 LST'
		opt_22.in_ext =		'.lst'		; input file extension
		opt_22.request =	'Select Fastcom MPA4 file to sort for all spectra'
		...
		opt_22.IC_mode = 	2			; default to indirect ion-beam charge mode using a "conv"
...
function FASTCOM_MPA4_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'FASTCOM_MPA4_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
; Sort Option items appear as device setup options in the Sort EVT "Device" tab
	...
	self.options.scan.ysize = 110							; Y size of sort options box, when open

	self.sort_options.charge_adc_mode = 1					; use ADC 2 (mode=1, ste=1) initially.
	self.sort_options.charge_smooth_mode = 1				; use smooth 2 initially (out of none, 2,3,4,5,...)
	self.sort_options.dead_smooth_mode = 3					; use smooth 4 initially (out of none, 2,3,4,5,...)

; Initial heap allocation for sort_id widget vector pointers ...

	self.sort_id.charge_adc_id = ptr_new(/allocate_heap)	; charge ADC mode ID array pointer
	self.sort_id.charge_smooth_id = ptr_new(/allocate_heap)	; charge smooth mode ID array pointer
	self.sort_id.dead_smooth_id = ptr_new(/allocate_heap)	; dead smooth mode ID array pointer

;	Pass core device parameters to base DEVICE superclass
;	Note that 'name' must match the object's file-name:
;	"FASTCOM_MPA4_DEVICE" --> FASTCOM_MPA4_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'FASTCOM_MPA4_DEVICE', $	; unique name for this device object
		title = 'Fastcom MPA4 - PC data acquisiion', $
...
pro fastcom_MPA4_device__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('FASTCOM_MPA4_DEVICE')

COMPILE_OPT STRICTARR

maia = {FASTCOM_MPA4_DEVICE,  $

		INHERITS BASE_DEVICE, $		; mandatory base device parameters

		sort_options : {sort_options_devicespec_mpa4, $		; Sort EVT window Sort options panel
				charge_adc_mode:	0, $					; ADC to use for charge pulses (starts at 0)
				charge_smooth_mode:	0, $					; none, 2,3,4,5,...
				dead_smooth_mode:	0}, $					; none, 2,3,4,5,...

											
		sort_id: {sort_id_mpa4, $							; pointers to vector of sort widget IDs
				charge_adc_id:		ptr_new(), $			; charge ADC mode ID array pointer
				charge_smooth_id:	ptr_new(), $			; charge smooth mode ID array pointer
				dead_smooth_id:	ptr_new()} $				; dead-fraction smooth mode ID array pointer

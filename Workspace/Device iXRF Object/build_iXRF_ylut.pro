function build_iXRF_ylut, file, output=output, slow_axis=slow_axis, error=error
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       warning,'build_iXRF_ylut',['IDL run-time error caught.', '', $
...
print,'build_iXRF_ylut: Scan blog files for Y lookup table data ...'
error = 1
...
for i=0,nq-1 do begin
	...
	if first then begin
		...
		openr, lun, fs[q[i]]
	endelse
	p = read_iXRF_segments( fs[q[i]], n_buffer=n_buffer, unit=lun, /veto_progress)
	if ptr_valid(p[0]) then begin
		pp = ptr_new( p)
		d = get_iXRF_details( pp, error=err)
...
write_iXRF_ylut, ylut, file[0], output=output, /strip, error=error

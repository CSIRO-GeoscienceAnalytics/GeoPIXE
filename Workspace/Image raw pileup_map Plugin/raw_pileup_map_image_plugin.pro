;
;  raw pileup_map Image plugin routine
;  -----------------------------
;
...
pro raw_pileup_map_image_plugin, p, i, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = 'Pileup loss map Image Plugin'	; return the menu title for this plugin
	return
endif
...
if (*p).has_pileup then begin
	pileup_map = *(*p).pileup_map
	name = 'PU'

	q = where( (pileup_map) ge 0.5*max(pileup_map), nq)
	q2 = where( ((pileup_map) lt 0.5*max(pileup_map)) and ((pileup_map) gt 0.), nq2)
	q3 = where( (pileup_map) eq 0., nq3)
	print, 'average pileup_map = ', mean(pileup_map)
...
;	No need to add the plugin name here; this is done by GeoPIXE.
	...
	history = 'pileup_map found'
endif else begin
	print,'Pileup loss map plugin: No "pileup_map" array found.'
	history = 'No pileup_map found'
endelse

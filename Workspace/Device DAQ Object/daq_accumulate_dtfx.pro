pro daq_accumulate_dtfx, t,x,y,ste,veto,fx,n,n_fx, flux_mode, flux_scale, xrange,yrange,n_flux, $
	dta,dtb,dtc, dead, flux=flux, dwell=dwell, xcompress=xcompress, ycompress=ycompress, $
	spectrum=spectrum, image=image, pseudo=pseudo, error=err
;
;  This assumes x,y,t,veto,pseudo	uintarr(n) (pseudo only used for /image)
;				fx			fltarr(n_fx,n)
;				flux_mode	0=Epics PV, 1=FC0, 2=FC1
;				flux_scale	units for flux scaling
;				dta,dtb		dead-time calibration (ms) for SCEPTER channels (0-31)
;				dtc			dead-time calibration (ms) for NIM channels (32-35)
...
;				xrange,1	dimensions for dead spectrum mode
	...
	if catch_errors_on then begin
		...
		if (ErrorNo ne 0) then begin
			...
			warning,'daq_accumulate_dtfx',['IDL run-time error caught.', '', $
	...
	if size(dtc,/tname) ne 'FLOAT' then begin sb='dtc' & goto, bad & endif
	...
	err = call_external( geopixe_library(), geolib_name( 'daq_accumulate_dtfx'), cdecl=geolib_cdecl(), $
		image, t,x,y,ste,veto,pseudo,fx,long(n),long(n_fx), long(flux_mode), flux_scale, $
		long(xcompress),long(ycompress), long(xrange),long(yrange),long(n_flux), $
		dta,dtb,dtc, flux, dead, dwell, value=value )
	if err then gprint,' daq_accumulate_dtfx: Error in Fortran - bad number of arguments'
	return

	bad:
	gprint,' daq_accumulate_dtfx: Error - bad type for argument: '+sb
	err = 1L
	return
end

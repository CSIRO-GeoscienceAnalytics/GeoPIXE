;
; GeoPIXE Device Object for Tohuku Labo system data
;
; Initialization:
...
pro TOHOKU_LABO_DEVICE::cleanup
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'TOHOKU_LABO_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
function TOHOKU_LABO_DEVICE::read_setup, unit, xrange,yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'TOHOKU_LABO_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
;		"read_buffer".
		...
		mpa_x_adc = 2					; set these ONLY FOR TESTING !!
		mpa_y_adc = 3

		n_buffer = 100000L				; ~100K x 4 byte buffers (0.4 MBytes)
;		n_buffer = 16L
		stat = fstat(unit)
		n_buffer = n_buffer < ((stat.size) / 4)
		n_guide = n_buffer
		i_buffer = 0L
		j_mpa = 0L
		ibranch = 1L
...
;		print,' MPA3 X,Y ADCs = ',mpa_x_adc, mpa_y_adc
...
function TOHOKU_LABO_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'TOHOKU_LABO_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_mpsys_4b, event_array, buffer, n_buffer,i_buffer
	...
	nc = n_elements(channel_on)
       ...
       read_event_buffer, unit, self, n_actual, bytes=2
       ...
       err = tohoku_events( event_array,j_mpa,ibranch,n_buffer,n_actual, $
          e,x1,y1,ste, n,n_buffer, mpa_x_adc,mpa_y_adc, $
          lword, ADCindex, ADCpntr, adc,tag, k_adc, adcs, bad_xy)

       if err ne 0 then begin
         print,'read_buffer: error (',err,') return from tohoku_events'
         goto, bad_io
       endif
       good = n

       if good gt 0 then begin
	       q  = where(e[0:good-1] gt 9000, cq)
	       if cq gt 0 then begin
	         print, 'found ',cq,' bad E in buffer ',i_buffer
	       endif
         ...
         if nqc gt 0 then begin
          ...
          if nq gt 0 then begin
              if raw_xy eq 0 then begin
                 ...
                 x1 = ((x1[qc[q]] - minx) > 0) / uint(xcompress)
                 y1 = ((y1[qc[q]] - miny) > 0) / uint(ycompress)
              endif else begin
                 e = e[qc[q]]
                 x1 = x1[qc[q]]
                 y1 = y1[qc[q]]
...
function TOHOKU_LABO_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'TOHOKU_LABO_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'tohoku_labo_evt': begin						; 28
			warning,'TOHOKU_LABO_DEVICE::import_spec',['"tohoku_labo_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p
end
...
function TOHOKU_LABO_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_28 = define(/import)			; Tohoku LST
		opt_28.name =		'tohoku_labo_evt'		; unique name of import
		opt_28.title =		'Extract from Tohoku LST'
		opt_28.in_ext =		'.lst'		; input file extension
		opt_28.request =	'Select Tohoku LST file to sort for all spectra'
		opt_28.raw =		1			; flags use of separate Raw data path '(*pstate).dpath'
		opt_28.spec_evt =	1			; uses a call to spec_evt to extract from EVT data
...
function TOHOKU_LABO_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'TOHOKU_LABO_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"TOHOKU_LABO_DEVICE" --> TOHOKU_LABO_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'TOHOKU_LABO_DEVICE', $	; unique name for this device object
		title = 'Tohoku Labo - PC data acquisition', $
		ext = '.lst', $			; file extension for raw data
...
pro TOHOKU_LABO_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('TOHOKU_LABO_DEVICE')

COMPILE_OPT STRICTARR

maia = {TOHOKU_LABO_DEVICE,  $

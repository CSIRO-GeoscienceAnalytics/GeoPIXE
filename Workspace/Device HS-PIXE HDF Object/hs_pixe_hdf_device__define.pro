...
;
; name()				return name of device (e.g. "HS_PIXE_HDF_DEVICE").
...
pro HS_PIXE_HDF_DEVICE::cleanup
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'HS_PIXE_HDF_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
function HS_PIXE_HDF_DEVICE::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_HDF_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	if H5F_IS_HDF5(strip_file_ext(file)+'.h5') eq 0 then begin
		warning,'HS_PIXE_HDF_DEVICE::get_header_info','Not a valid HDF5 file: '+strip_file_ext(file)+'.h5'
		self.header.error = 1
		return, self.header
	endif
...
self.header.detector[0:n-1] = 0          ; PIXE
...
self.header.scan.x_pixels = 264
self.header.scan.y_pixels = 264
self.header.scan.on = 1
self.header.scan.x_mm = 12
self.header.scan.y_mm = 12
...
pro HS_PIXE_HDF_DEVICE::flux_scan, unit, evt_file, PV_list, IC_name, IC_val, IC_vunit, dwell=dwell, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_HDF_DEVICE::flux_scan',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
bad_io:
	warning,'HS_PIXE_HDF_DEVICE::flux_scan','HDF file I/O error.'
	error = 1
	return
end
...
function HS_PIXE_HDF_DEVICE::read_setup, unit, xrange, yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_HDF_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
;
		...
		skip_id = H5D_OPEN(hdf5_id, '/Skip')
		hit_id = H5D_OPEN(hdf5_id, '/Hit')


      	dead_fraction = float( H5D_READ(skip_id)) * 1. /  float( H5D_READ(hit_id))
...
;;	This will take up a lot of memory (approaching 1 Gb) for individual detectors.
 		...
 		if s gt 300 then begin
 			warning,'HS_PIXE_HDF_DEVICE::read_setup','array in memory will be ',s,'Mbytes in size ...'
 		endif

	    hs_pixe_read_line = 0
		hdf5_spectra = H5D_READ(spectra_id)
...
function HS_PIXE_HDF_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_HDF_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
;			print,'x,y=',hs_pixe_x,hs_pixe_y
	   ...
	   if read_by_line and hs_pixe_read_line then begin
			...
			start = [0,0,i_buffer]
			count = [hs_pixe_chans,hs_pixe_x_range,0]
...
;	       multiple = long( reform(hdf5_spectra[hs_pixe_x,*], hs_pixe_chans*hs_pixe_dets))
	       multiple = long( reform(hdf5_spectra[*,hs_pixe_x,0], hs_pixe_chans))
	   endif else begin
	       multiple = long( reform(hdf5_spectra[*,hs_pixe_x,hs_pixe_y], hs_pixe_chans))
	   endelse
...
function HS_PIXE_HDF_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_HDF_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	case name of
		'hs_pixe_hdf_evt': begin						; 41
		  ...
		  p2 = ptrarr(1)
		  p2[0] = ptr_new(spec)
			;warning,'HS_PIXE_HDF_DEVICE::import_spec',['"hs_pixe_hdf_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
			end
	endcase
	return, p2
end
...
function HS_PIXE_HDF_DEVICE::get_import_list, error=error
...
;		yr:				200 $			; default Y range
	...
	opt_41 = define(/import)			; HS_PIXE HDF image file
		opt_41.name =		'hs_pixe_hdf_evt'		; unique name of import
		opt_41.title =		'Extract from HS-PIXE HDF file'
		opt_41.in_ext =		'.h5'		; input file extension
		opt_41.request =	'Select HS-PIXE HDF file to extract the spectrum'
	;	opt_41.preview = 1     ; allow spectrum preview
	...
	;  opt_41.xr =     264      ; default X range
  ;  opt_41.yr =     264      ; default Y range
...
function HS_PIXE_HDF_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'HS_PIXE_HDF_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"HS_PIXE_HDF_DEVICE" --> HS_PIXE_HDF_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'HS_PIXE_HDF_DEVICE', $	; unique name for this device object
		title = 'HS-PIXE image HDF data file', $
...
pro HS_PIXE_HDF_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('HS_PIXE_HDF_DEVICE')

COMPILE_OPT STRICTARR

maia = {HS_PIXE_HDF_DEVICE,  $

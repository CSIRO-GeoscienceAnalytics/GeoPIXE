;
;  Peak Ni Centre Spectrum plugin routine
;  
...
pro Peak_Ni_Centre_spectrum_plugin, p, ii, marks, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = '* Peak Ni Centre Plugin'		; return the menu title for this plugin
...
if cunits eq 'keV' then begin
	...
	elow = ca*mark[0]+cb
	ehigh = ca*mark[1]+cb
	emn = 7.47								; target centroid of Fe Ka + b
endif else begin
	elow = mark[0]
	ehigh = mark[1]
	emn = 932.								; expected channel number
endelse

...
;
; name()				return name of device (e.g. "chess_hdf_DEVICE").
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'chess_hdf_DEVICE::cleanup',['IDL run-time error caught.', '', $
...
pro chess_hdf_device::set_options, p, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_device::set_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
function chess_hdf_device::get_options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_device::get_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
pro chess_hdf_device::read_options, unit, options=options, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_device::read_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
pro chess_hdf_device::write_options, unit, options=p, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_device::write_options',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
function chess_hdf_device::get_header_info, file, output=output, silent=silent, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_DEVICE::get_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
common c_geopixe_adcs, geopixe_max_adcs
	...
	if H5F_is_hdf5(file) eq 0 then begin
		warning,'chess_hdf_DEVICE::get_header_info','Not a valid HDF5 file: '+file
		self.header.error = 1
		return, self.header
	endif
	
	mp = get_chess_hdf_header( file, error=error)
	if error then begin
		self.header.error = 1
		return, self.header
	endif
...
pro chess_hdf_DEVICE::update_header_info, error=error
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_DEVICE::update_header_info',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return
...
common c_chess_12, chess_flux
	...
	self.header.scan.x_mm = (*pmp).sx
	self.header.scan.y_mm = (*pmp).sy
	self.header.scan.x = (*pmp).orgx
	self.header.scan.y = (*pmp).orgy
...
;	self.header.metadata.endstation = (*pmp).endstation
	...
	chess_flux = (*pmp).flux								; stored flux array from header
...
;pro chess_hdf_DEVICE::check_pv_list, plist
;
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'chess_hdf_DEVICE::check_pv_list',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		error = 1
...
;pro chess_hdf_DEVICE::flux_scan, unit, evt_file, PV_list, IC_name, IC_val, IC_vunit, dwell=dwell, $
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'chess_hdf_DEVICE::flux_scan',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		return
...
;	error = 1
;	
;	mp = read_chess_hdf_header( unit, error=error)
...
;
;bad_io:
;	warning,'chess_hdf_DEVICE::flux_scan','HDF file I/O error.'
;	error = 1
;	return
;end
...
function chess_hdf_DEVICE::read_setup, unit, xrange, yrange, first=first, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_DEVICE::read_setup',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_chess_11, i_spectra, q_spectra, name_spectra
common c_chess_12, chess_flux
common c_nsls_5, nsls_IC_value_index, nsls_flux_scale
	...
	if first then begin
		head = read_chess_hdf_header( unit, error=err32)	
	...
	base_id = H5G_OPEN(hdf5_id,'entry/instrument/detector')
	...
	nm = H5G_get_nmembers(hdf5_id,'entry/instrument/detector')	
	name_spectra = strarr(nm)
	for i=0,nm-1 do begin
		name_spectra[i] = H5G_get_member_name(hdf5_id,'entry/instrument/detector',i)
	endfor

	q = where( name_spectra eq 'data', nq)
	if nq eq 0 then begin
		warning,'chess_hdf_DEVICE::read_setup','No "data" dataset found in "entry/instrument/detector".'
		H5F_close, hdf5_id
		goto, bad_io
	endif

	det_id = H5D_OPEN(hdf5_id,'entry/instrument/detector/data')

	dspace = H5D_get_space(det_id)
	dims = H5S_get_simple_extent_dims(dspace)
	H5S_close, dspace

	hdf5_spectra = H5D_read(det_id)
	H5D_close, det_id

	n_chans = dims[0]
	n_det = dims[1]
	nx = dims[2]
	ny = n_elements( chess_flux[0,*])
	self.fresh = 1
...
;	endelse
	...
	progress_file = 1													; progress indicator by file
	progress_size = dims[2] * n_det

  	pnc_hdf_x_range = nx
  	pnc_hdf_y_range = ny
  	pnc_hdf_dets = n_det
  	pnc_hdf_chans = n_chans
  	pnc_hdf_x = 0
	if first then pnc_hdf_y = 0

;	Set up various vectors to use to construct X,Y,E events in 'read_buffer'

	ramp = indgen(pnc_hdf_chans)
	pnc_hdf_e = uintarr( pnc_hdf_chans, pnc_hdf_dets, pnc_hdf_x_range)
	pnc_hdf_x1 = pnc_hdf_e
	pnc_hdf_ste = pnc_hdf_e
	for i=0L,pnc_hdf_dets-1 do begin
		pnc_hdf_ste[*,i,*] = i
		for j=0L,pnc_hdf_x_range-1 do begin
		 	 pnc_hdf_e[*,i,j] = ramp
		 	 pnc_hdf_x1[*,i,j] = j
		endfor
	endfor
	pnc_hdf_e = reform( pnc_hdf_e, pnc_hdf_chans*pnc_hdf_dets*pnc_hdf_x_range)
	pnc_hdf_ste = reform( pnc_hdf_ste, pnc_hdf_chans*pnc_hdf_dets*pnc_hdf_x_range)
	pnc_hdf_x1 = reform( pnc_hdf_x1, pnc_hdf_chans*pnc_hdf_dets*pnc_hdf_x_range)
	pnc_hdf_y1 = uintarr( pnc_hdf_chans*pnc_hdf_dets*pnc_hdf_x_range)
	return, 0
...
function chess_hdf_DEVICE::read_buffer, unit, x1,y1,e, channel_on,n, xcompress,ycompress, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_DEVICE::read_buffer',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 1
...
common c_nsls_5, nsls_IC_value_index, nsls_flux_scale
	...
	good = 0
	n = 0
	if self.fresh eq 0 then goto, bad_io
...
;	Spectum count per pixel/channel used to set a 'multiplicity' for each 'event'.
	...
	multiple = long( reform(hdf5_spectra, pnc_hdf_chans*pnc_hdf_dets*pnc_hdf_x_range))
...
;function chess_hdf_DEVICE::get_dwell, error=error
;
...
;		c = 'Call stack: '
;		if n gt 2 then c = [c, s[1:n-2]]
;		warning,'chess_hdf_DEVICE::get_dwell',['IDL run-time error caught.', '', $
;				'Error:  '+strtrim(!error_state.name,2), $
;				!error_state.msg,'',c], /error
;		MESSAGE, /RESET
;		error = 1
...
function chess_hdf_DEVICE::import_spec, name, file, group=group
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_DEVICE::get_import_list',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
case name of
	'chess_hdf_spec': begin						; 41
		p2 = 0L
		if H5F_is_hdf5( file) eq 0 then begin 
			warning,'chess_hdf_DEVICE::get_import_list','Not a valid HDF5 file: '+file
			goto, chess_hdf_evt_done
		endif
		...
		if nq eq 0 then begin
			warning,'chess_hdf_DEVICE::get_import_list','No "MCA" datasets found in group.'
			H5F_close, file_id
			goto, chess_hdf_evt_done
		endif
...
chess_hdf_evt_done:
	...
	'chess_hdf_evt': begin							; 9
		warning,'chess_hdf_DEVICE::import_spec',['"chess_hdf_evt" spectrum import.','This import should use "spec_evt" elsewhere.']
		end
	endcase
	return, p2
end
...
function chess_hdf_DEVICE::get_import_list, error=error
...
;	opt_41 = define(/import)			; CHESS new HDF5 image file
;		opt_41.name =		'chess_hdf_spec'		; unique name of import
;		opt_41.title =		'Extract E from chess image HDF5 file'
;		opt_41.in_ext =		'.hdf5'		; input file extension
;		opt_41.request =	'Select chess HDF file to scan for all spectra'
;		opt_41.spec_evt =	0			; uses direct HDF5 reads for spectra
;		opt_41.use_IC =		0			; pop-up the flux_select PV selection panel
;		opt_41.IC_mode = 	2			; default to using PV for IC
	
	opt_39 = define(/import)			; CHESS new HDF5 file read as a list-mode
		opt_39.name =		'chess_hdf_evt'	; unique name of import
		opt_39.title =		'Extract E,X,Y from chess HDF5 files as list-mode'
		...
		opt_39.request =	'Select chess HDF files to scan for all spectra, X,Y'
...
function chess_hdf_DEVICE::init
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		c = 'Call stack: '
		if n gt 2 then c = [c, s[1:n-2]]
		warning,'chess_hdf_DEVICE::init',['IDL run-time error caught.', '', $
				'Error:  '+strtrim(!error_state.name,2), $
				!error_state.msg,'',c], /error
		MESSAGE, /RESET
		return, 0
...
;	"chess_hdf_DEVICE" --> chess_hdf_DEVICE_define.sav
;	and 'name' must contain the string "_DEVICE".
	 
	i = self->BASE_DEVICE::init(  $
		name = 'CHESS_HDF_DEVICE', $	; unique name for this device object
		title = 'CHESS image data HDF5 files', $
...
pro chess_hdf_DEVICE__define

; Define Maia device object internal data structure.
; Only called using obj = obj_new('chess_hdf_DEVICE')

COMPILE_OPT STRICTARR

maia = { chess_HDF_DEVICE,  $

		INHERITS BASE_DEVICE, $			; mandatory base device parameters

		fresh:	0, $					; flags a fresh new file read, for read_buffer
...
;		write and read of device options, which is defined in the 'write_options' method.
		...
		sort_options : {sort_options_devicespec_chess_hdf, $	; Sort EVT window Sort options panel

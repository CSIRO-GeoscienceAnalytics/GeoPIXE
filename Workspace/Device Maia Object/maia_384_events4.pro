function maia_384_events4, ev,n_buffer, channel_on,nc, e,t,x,y,ste,veto,tags,n_events,n, $
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'maia_384_events4',['IDL run-time error caught.', '', $
...
if size(skip,/tname) ne 'INT' then begin sb='skip' & goto, bad & endif
...
err = call_external( geopixe_library(), geolib_name( 'maia_384_events4'), cdecl=geolib_cdecl(), $
...
if err then begin
	print, 'maia_384_events4 Fortran error'
	print,'		tag, length, ibranch, skip =',tag, length, ibranch, skip
endif
return, err

bad:
	gprint,'maia_384_events4: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

function maia_384_events6, ev,n_buffer, channel_on,nc, e,t,x,y,z,ste,veto,tags,n_events,n, $
			fx,n_fx, flux_mode, x0,y0,z0, monitor,n_monitor,nm, $
			energy_string, beam_energy, flux_string, flux_pv, time_pv, time_first, $
			y_mode,xmargin,ymargin,width,height, ibranch,swap, tag,length,skip, bad,debug
;
...
;				e,t,ste		E,T,n for events intarr(n_events)
;				x,y,z		X,Y,Z for events lonarr(n_events)
...
;				
;				y_mode		1 new Y encoder mode, 0 normal PA advance, 2 de-glitch +/- 16, 3 YZ encoder mode
;				xmargin, ymargin	width of X,Y margin for Y(Z) encoder mode
;				width, height		height of scan for Z encoder mode
;
...
;				x0,y0,z0	long	current X,Y,Z pixel position (internal)
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'maia_384_events6',['IDL run-time error caught.', '', $
...
if size(x,/tname) ne 'LONG' then begin sb='x' & goto, bad & endif
if size(y,/tname) ne 'LONG' then begin sb='y' & goto, bad & endif
if size(z,/tname) ne 'LONG' then begin sb='z' & goto, bad & endif
...
if size(x0,/tname) ne 'LONG' then begin sb='x0' & goto, bad & endif
if size(y0,/tname) ne 'LONG' then begin sb='y0' & goto, bad & endif
if size(z0,/tname) ne 'LONG' then begin sb='z0' & goto, bad & endif
...
err = call_external( geopixe_library(), geolib_name( 'maia_384_events6'), cdecl=geolib_cdecl(), $
			ev,long(n_buffer), channel_on,long(nc), e,t,x,y,z,ste,veto,tags,long(n_events),n, $
			fx,long(n_fx), long(flux_mode), x0,y0,z0, monitor,long(n_monitor),nm, $
			energyb,nb, beam_energy, fluxb,nf, flux_pv,time_pv, time_first, $
			long(y_mode),long(xmargin),long(ymargin),long(width),long(height), ibranch, long(swap), tag,length,skip, bad,debug, $ 
			value=value )

if err then begin
	print, 'maia_384_events6 Fortran error'
	print,'		tag, length, ibranch, skip =',tag, length, ibranch, skip
endif
return, err

bad:
	gprint,'maia_384_events6: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

...
pro raw_flux_image_plugin, p, i, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = 'raw flux Image Plugin'			; return the menu title for this plugin
	return
endif
...
if (*p).has_flux then begin
	...
	!p.title = 'Raw Flux/Charge distribution'
	...
	(*pimg)[*,*,0] = flux	
	(*(*p).el)[0] = name
	history = 'raw flux found'				; history return
endif else begin
	print,'raw flux plugin: No "raw flux" array found.'
	history = 'No raw flux found'
endelse

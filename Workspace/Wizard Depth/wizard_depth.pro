			...
			'wizard-return': begin	
				if ptr_valid( event.pointer) then begin
					...
					if (*pw).wizard ne 'depth' then begin
						...
						warning, 'wizard_depth_return', ['Another Wizard appears to be open ("'+(*pw).wizard+'").', '', $
...
;					increment a count to test if unwanted duplicates are open.
					...
					if (*pw).command eq 'open-test' then begin
						...
						if count gt 1 then begin
							...
							warning, 'wizard_depth_return', ['Multiple windows of types "'+strjoin((*pstate).windows_needed,', ')+'" may be open.', '', $

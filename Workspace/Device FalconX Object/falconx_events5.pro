function falconx_events5, ev,n_buffer, channel_on,nc, e,t,x,y,z,u,v,w,ste,veto,tags,n_events,n, $
			fx,n_fx, flux_mode, swap_icr_raw, x0,y0,z0,u0,v0,w0, xy_correct,xmargin,width, $
			ibranch,swap, tag,length,skip, bad,debug, version1,tick1
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'falconx_events5',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'falconx_events5'), cdecl=geolib_cdecl(), $
			ev,long(n_buffer), channel_on,long(nc), e,t,x,y,z,u,v,w,ste,veto,tags,long(n_events),n, $
			fx,long(n_fx), long(flux_mode), long(swap_icr_raw), x0,y0,z0,u0,v0,w0, $
			long(xy_correct), long(xmargin), long(width), ibranch, long(swap), tag,length,skip, bad,debug, $ 
...
bad:
	gprint,'falconx_events5: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

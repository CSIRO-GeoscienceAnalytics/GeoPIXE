function falconx_events1, ev,n_buffer, channel_on,nc, e,t,x,y,z,ste,veto,tags,n_events,n, $
			fx,n_fx, flux_mode, x0,y0,z0, ibranch,swap, tag,length,skip, bad,debug
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'falconx_events1',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'falconx_events1'), cdecl=geolib_cdecl(), $
			ev,long(n_buffer), channel_on,long(nc), e,t,x,y,z,ste,veto,tags,long(n_events),n, $
			fx,long(n_fx), long(flux_mode), x0,y0,z0, ibranch, long(swap), tag,length,skip, bad,debug, $ 
			value=value )

if err then begin
	print, 'falconx_events1 Fortran error'
	print,'		tag, length, ibranch, skip =',tag, length, ibranch, skip
endif
return, err

bad:
	gprint,'falconx_events1: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

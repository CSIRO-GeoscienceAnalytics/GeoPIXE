...
if size( plist, /tname) eq 'POINTER' then begin
	...
	list = ['FalconX:scaler.FC0', 'FalconX:scaler.FC1', 'FalconX:scaler.FC2', 'FalconX:scaler.FC3', list]
	
	q = where( strmid(list,0,4) eq 'none', nq)
	if nq eq 0 then begin
		list = [list, 'none']
	endif
	q = where( strmid(list,0,14) eq 'FalconX:scaler', nq)
	if nq gt 4 then begin
		list[q[4:*]] = ''
	endif
	q = where( (list ne ''), nq)

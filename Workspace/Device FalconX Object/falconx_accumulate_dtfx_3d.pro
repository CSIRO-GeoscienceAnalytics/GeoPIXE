pro falconx_accumulate_dtfx_3D, x,y,z,ste,veto,fx,n,n_fx, flux_mode, flux_scale, xrange,yrange,zrange, $
			dead, flux=flux, dwell=dwell, xcompress=xcompress, ycompress=ycompress, $
			spectrum=spectrum, image=image, pseudo=pseudo, weight=weight, error=err
;
;  This assumes x,y,z,veto,pseudo	uintarr(n) (pseudo only used for /image)
;				fx			fltarr(n_fx,n)
;				flux_mode	0=Epics PV, 1=FC0, 2=FC1, 3=FC2, 4=FC3
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'falconx_accumulate_dtfx_3D',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'falconx_accumulate_dtfx_3d'), cdecl=geolib_cdecl(), $
		image, x,y,z,ste,veto,pseudo,fx,long(n),long(n_fx), long(flux_mode), flux_scale, $
		long(xcompress),long(ycompress), long(xrange),long(yrange),long(zrange), $
		flux, dead, weight, dwell, value=value )
if err then gprint,' falconx_accumulate_dtfx_3D: Error in Fortran - bad number of arguments'
return

bad:
	gprint,' falconx_accumulate_dtfx_3D: Error - bad type for argument: '+sb
	err = 1L
	return
end

function falconx_events3, ev,n_buffer, channel_on,nc, e,t,x,y,z,u,v,w,ste,veto,tags,n_events,n, $
			fx,n_fx, flux_mode, x0,y0,z0,u0,v0,w0, ibranch,swap, tag,length,skip, bad,debug, version
;
...
if catch_errors_on then begin
	...
	if (ErrorNo ne 0) then begin
		...
		warning,'falconx_events3',['IDL run-time error caught.', '', $
...
err = call_external( geopixe_library(), geolib_name( 'falconx_events3'), cdecl=geolib_cdecl(), $
...
if err then begin
	print, 'falconx_events3 Fortran error'
	print,'		tag, length, ibranch, skip =',tag, length, ibranch, skip
endif
return, err

bad:
	gprint,'falconx_events3: Error - bad type for argument: '+sb
	err = 1L
	return, err
end

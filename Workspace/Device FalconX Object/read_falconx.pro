function read_falconx, unit, n_buffer=n_bufferi, state=state, n_actual=n_actual, version=version, $
					progressbar=do_progress, select=select, skip=skip, accept=accept, njson=njson
...
; njson		length of a (pre-read) JSON header string
...
if catch_errors_on then begin
    ...
    if (ErrorNo ne 0) then begin
       ...
       warning,'read_falconx',['IDL run-time error caught.', '', $
...
if n_elements(n_bufferi) lt 1 then n_bufferi = 50000	;	200000LL
...
if n_elements(version) lt 1 then version=999999L
if n_elements(accept) lt 1 then accept=indgen(1000)
use = intarr(1000)
use[accept] = 1

if select ge 0 then do_progress=0
if do_progress then progress, tlb=progress_tlb, title='Read FalconX'
...
new_block:
	if do_progress and (iprog ge nprog) then begin
		...
		progress, /update, progress_tlb, {unit:0, value:0, current:i, size:n_actual}, cancel=cancel, skip=skipbutton
		iprog = 0
		t = systime(/seconds)
		ft = t - ltime
		dt = t - time
...
more:
    i = i + n_head + record_length
    if i lt 0 then warning,'read_falconx','negative byte counter detected.'
	if (i lt n_actual-n_head-1) and (select eq -1) then begin
;	if (i lt n_actual-n_head) and (select eq -1) then begin
	    n_loop = n_loop+1
	    goto, new_block
	endif
	...
	if do_progress then begin
		wait,0.3
		progress, /complete, progress_tlb, 'Read falconx completed.'

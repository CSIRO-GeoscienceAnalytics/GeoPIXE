;
;  Peak mn Centre Spectrum plugin routine
;  
...
pro Peak_Mn_Centre_spectrum_plugin, p, ii, marks, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = '* Peak Mn Centre Plugin'		; return the menu title for this plugin
...
if cunits eq 'keV' then begin
	...
	elow = ca*mark[0]+cb
	ehigh = ca*mark[1]+cb
	emn = 6.01								; target centroid of Mn Ka + b
endif else begin
	elow = mark[0]
	ehigh = mark[1]
	emn = 735.								; expected channel number
endelse

;
;  blocky Image plugin routine
;  -----------------------------
;
;  All Image blocky routines MUST be named with "_image__plugin.pro"
...
pro blocky_image_plugin, p, i, title=title, history=history

COMPILE_OPT STRICTARR

if arg_present(title) then begin
	title = 'Blocky Image Plugin'			; return the menu title for this plugin
	return
endif
...
;r = 5.5										; reduce pixel count by 'r' on each axis
r = 10.										; reduce pixel count by 'r' on each axis

new_img = congrid( old_img, sx/r, sy/r)		; reduce without interp
new_img = congrid( new_img, sx, sy)			; rescale back to size

...
pro linearize_cuts_energies_spectrum_plugin, p, i, marks, title=title, history=history

COMPILE_OPT STRICTARR
common c_non_linearity3, centre_spec

if arg_present(title) then begin
	title = 'Build Linearize using CUTs labelled "Energy (keV)" Plugin'			; return the menu title for this plugin
...
eview = cview * (*p[0]).cal.poly[1] + (*p[0]).cal.poly[0]		; in "energy" (gain trimmed) units
...
; Set-up cuts over these peaks ...
; Cut labels must have format "label energy".

for j=0,nc-1 do begin
	str = strsplit( (*pcuts)[j].el, ' ', /extract)
	ns = n_elements(str)
	case ns of
		1: begin
			if gnumeric(str[0]) eq 0 then goto, bad_cut_energies
			energy[j] = float(str[0])
			end
		2: begin
			if gnumeric(str[1]) eq 0 then goto, bad_cut_energies
			energy[j] = float(str[1])
			end
		else: goto, bad_cut_energies
	endcase
endfor
...
;	history = 'Build Linearize plugin   [linearize_fixed_cuts_spectrum_plugin]'		; a history record for this plugin
	history = 'Linear file saved   [linearize_fixed_cuts_spectrum_plugin]'			; a history record for this plugin
...
bad_view:
	warning,'linearize_cuts_energies_spectrum_plugin','Need to select a range with View first.'
	goto, finish
bad_cuts:
	warning,'linearize_cuts_energies_spectrum_plugin','Bad cuts read'
	goto, finish
bad_cut_number:
	warning,'linearize_cuts_energies_spectrum_plugin','Wrong number of Cuts'
	goto, finish
bad_cut_energies:
	warning,'linearize_cuts_energies_spectrum_plugin',['CUT labels must contain line energy.','Express CUT labels as either:', $
				'(i) a "label" followed by "energy" after a <space>, or', '(ii) just an "energy".']
	goto, finish
bad_linear:
	warning,'linearize_cuts_energies_spectrum_plugin','Bad open/write of linearize file '+lfile
	goto, finish
bad_peaks:
	warning,'linearize_cuts_energies_spectrum_plugin',['Too few peaks to work with in View range', $
